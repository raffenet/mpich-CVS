...
program will have Process-Thread viewmap and Communicator-Thread viewmap
...
child thread count&
MPICH2%
\footnote{MPICH2-1.0.5p4 is built with default sock channel which has MPI\_THREAD\_MULTIPLE
support.%
}&
OpenMPI+progress\_thread%
\footnote{OpenMPI-1.2.3 is configured with --enable-mpi-threads and --enable-progress-threads.%
}&
OpenMPI%
\footnote{Since OpenMPI-1.2.3 by default does not support MPI\_THREAD\_MULTIPLE,
so --enable-mpi-threads is used to enable the thread\_multiple support.%
}\tabularnewline
\hline
\hline 
1&
...
threads in different MPI implementations.}
\end{figure}


Two Jumpshot views are shown with each run in Figures \ref{fig:openmpi_4_2},
\ref{fig:openmpi_4_3},\ref{fig:openmpi_4_4},\ref{fig:openmpi_4_5}
...
is always labeled as 0 in MPE logging and other duplicated MPI\_Comm
is labeled with other integer depends on the order of when it is being
created.
...
Alt-E to expand each process timeline to review the thread timeline.
Similarly, user can use the Y-axis LabelCollapse button \includegraphics{pic/gif2png/TreeCollapse24}
or Alt-C to collapse the thread timeline back to their corresponding
process timeline. Similarly for the communicator-thread view, the
Y-axis LabelExpand and LabelCollapse buttons should be used to expand
and collapse the communicator timelines.

Figures. \ref{fig:openmpi_4_3}, \ref{fig:openmpi_4_4}, \ref{fig:openmpi_4_5}
and \ref{fig:openmpi_4_6} clearly demonstrates that there is some
...
Communicator-Thread view, everything performs as expected.}
\end{figure}
...
two MPI\_Comm\_dup(). The expanded Communicator view also suggests
...
much earlier thann the communication in the last two communicators.}
\end{figure}
...
than previous MPI\_Comm\_dup() in finishing up. The feature that we
observed in Fig. \ref{fig:openmpi_4_3} that the delayed MPI\_Comm\_dup()
...
The following is the test program, pthread\_sendrecv.c, used in this
study.
