...
@*/
void CLOG_mergelogs( int shift, char *execfilename, int logtype )
{
...
@*/
void CLOG_treesetup( int self, int numprocs,
                     int *myparent, int *mylchild, int *myrchild)
{
    if (self == 0)
        *myparent = -1;
    else
...
void CLOG_csync( int root, double diffs[] )
{
    ...
    if (root == myrank) {        /* I am the master, but not nec. 0 */
        for (i = 0; i < numprocs; i++) {
            if (i != myrank) {
                ...
                for (j = 0; j < numtests; j++) {
                    PMPI_Send(&dummy, 0, MPI_INT, i, MASTER_READY,
                              MPI_COMM_WORLD);
                    PMPI_Recv(&dummy, 0, MPI_INT, i, SLAVE_READY,
                              MPI_COMM_WORLD, &status); 
                    time_1 = CLOG_timestamp();
                    PMPI_Send(&dummy, 0, MPI_INT, i, TIME_QUERY,
                              MPI_COMM_WORLD);
        ...
        for (j = 0; j < numtests; j++) {
            PMPI_Recv(&dummy, 0, MPI_INT, root, MASTER_READY,
                      MPI_COMM_WORLD, &status); 
            PMPI_Send(&dummy, 0, MPI_INT, root, SLAVE_READY, MPI_COMM_WORLD);
            PMPI_Recv(&dummy, 0, MPI_INT, root, TIME_QUERY,
                      MPI_COMM_WORLD, &status);
            time_i = CLOG_timestamp();
            PMPI_Send(&time_i, 1, MPI_DOUBLE, root, TIME_ANSWER,
                      MPI_COMM_WORLD);
        }
    }
...
void CLOG_printdiffs( double diffs[] )
{
    int i, numprocs, self;
