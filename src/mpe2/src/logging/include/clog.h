...
typedef char CLOG_COLOR[ 3 * sizeof(double) ];
typedef char CLOG_DESC[ 4 * sizeof(double) ];
typedef char CLOG_FILE[ 5 * sizeof(double) ];
typedef char CLOG_FORMAT[ 5 * sizeof(double) ];
typedef char CLOG_BYTES[ 4 * sizeof(double) ];

/* CLOG_STATE defines the attributes of a state which consists of 2 events */
typedef struct {
    int stateID;                /* integer identifier for state */
    int startetype;             /* beginning event for state */
    int finaletype;             /* ending event for state */
    int pad;
    CLOG_COLOR color;           /* string for color */
    CLOG_DESC name;             /* string describing state */
    CLOG_FORMAT format;         /* format string for state's decription data */
    double end[1];
} CLOG_STATE;

/* CLOG_EVENT defines the attritbutes of a event */
typedef struct {
    int etype;                  /* event ID */
    int pad;
    CLOG_COLOR color;           /* string for color */
    CLOG_DESC name;             /* string describing event */
    CLOG_FORMAT format;         /* format string for the event */
    double end[1];
} CLOG_EVENT;

/* CLOG_CONST defines some specific constant of the logfile */
typedef struct {
    int etype;                  /* raw event */
    int value;                  /* uninterpreted data */
    CLOG_DESC name;             /* uninterpreted string */
    double end[1];
} CLOG_CONST;
...
#define CLOG_STATEDEF     1     /* state description */
#define CLOG_EVENTDEF     2     /* event description */
#define CLOG_CONSTDEF     3     /* constant description */
#define CLOG_BAREEVENT    4     /* arbitrary record */
#define CLOG_CARGOEVENT   5     /* arbitrary record */
#define CLOG_MSGEVENT     6     /* message event */
#define CLOG_COLLEVENT    7     /* collective event */
#define CLOG_COMMEVENT    8     /* communicator construction/destruction  */
#define CLOG_SRCLOC       9     /* identifier of location in source */
#define CLOG_SHIFT       10     /* time shift calculated for this process */
...
#define CLOG_MSG_SEND -101
#define CLOG_MSG_RECV -102

/* special event type for defining constants */
#define CLOG_CONST_DEF -201
...
extern char        CLOG_outdir[CLOG_DIR_LEN];
...
extern double     *CLOG_out_buffer,    /* buffers for clog-merge (abhi) */
                  *CLOG_left_buffer,
                  *CLOG_right_buffer;
...
void CLOG_LOGSTATE( int, int, int, const char *, const char *, const char * );
void CLOG_LOGEVENT( int, const char *, const char *, const char * );
void CLOG_LOGCONST( int, int, const char * );
void CLOG_LOGBARE( int );
void CLOG_LOGCARGO( int, const char * );
void CLOG_LOGMSG( int, int, int, int, int );
void CLOG_LOGCOLL( int, int, int, int );
void CLOG_LOGCOMM( int, int, int );
void CLOG_LOGSRCLOC( int , int, const char * );
...
void CLOG_byteswap( void *, int, int );
