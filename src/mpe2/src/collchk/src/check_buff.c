/*
   (C) 2004 by Argonne National Laboratory.
       See COPYRIGHT in top-level directory.
*/
#include "collchk.h" 

int CollChk_check_buff(MPI_Comm comm, void * buff, char* call)
{
    /* rank, size, counter, temp comm buffer, go flag, ok flag */
    int r, s, i, tempbuff, go, ok;
    char err_str[255];           /* error string */
    MPI_Status st;int tag=0;     /* needed for communications */

    /* get the rank and size */
    MPI_Comm_rank(comm, &r);
    MPI_Comm_size(comm, &s);

    sprintf(err_str, "no error");

    if (r == 0) {
        /* if buff is MPI_IN_PLACE send a set flag else send an unset flag */
        if(buff == MPI_IN_PLACE) {
            tempbuff = 1;
        }
        else {
            tempbuff = 0;
        }

        PMPI_Bcast(&tempbuff, 1, MPI_INT, 0, comm);

        /* check if all processes are ok to continue */
        go = 1; /* set the go flag */
        for (i=1; i<s; i++) {
            MPI_Recv(&ok, 1, MPI_INT, i, tag, comm, &st);
            /* if the process is not ok unset the go flag */
            if (ok != 1) go = 0;
        }

        /* broadcast the go flag to the other processes */
        PMPI_Bcast(&go, 1, MPI_INT, 0, comm);
    }
    else {
        /* get the MPI_IN_PLACE flag from 0 */
        PMPI_Bcast(&tempbuff, 1, MPI_INT, 0, comm);

        /* check the local use of MPI_IN_PLACE against 0s */
        if ((tempbuff == 1) && (buff != MPI_IN_PLACE)) {
            /* at this point the call parameter is inconsistent */
            /* print an error message and send an unset ok flag to 0 */
            ok = 0;
            sprintf(err_str,
                    "Use of MPI_IN_PLACE is Inconsistent with Rank 0.\n");
            MPI_Send(&ok, 1, MPI_INT, 0, tag, comm);
        }
        else {
            /* at this point the call parameter is consistent  */
            /* send a set ok flag to 0 */
            ok = 1;
            MPI_Send(&ok, 1, MPI_INT, 0, tag, comm);
        }
        
        /* recieve the go flag from 0 */
        PMPI_Bcast(&go, 1, MPI_INT, 0, comm);
    }

    /* if the go flag is not set exit else return */
    if (go != 1) {
        return CollChk_err_han(err_str, COLLCHK_ERR_INPLACE, call, comm);
    }

    return MPI_SUCCESS;
}
