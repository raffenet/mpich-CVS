...
static HANDLE g_hListenReleasedEvent;

void InitRedirection()
{
    g_hListenReleasedEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
}

void FinalizeRedirection()
{
    CloseHandle(g_hListenReleasedEvent);
}
...
static void RedirectStdin(int bfd)
{
    ...
    if (hStdin == INVALID_HANDLE_VALUE)
    {
	err_printf("Critical error: Unable to acquire the standard input handle for redirection. error %d\n", GetLastError());
	fflush(stdout);
    }
...
void RedirectIOThread2(int abort_bfd)
{
	...
	if (n == SOCKET_ERROR)
	{
	    err_printf("RedirectIOThread2: bselect failed, error %d\n", WSAGetLastError());fflush(stdout);
	...
	if (n == 0)
	{
	    err_printf("RedirectIOThread2: bselect returned zero sockets available\n");fflush(stdout);
	    beasy_closesocket(abort_bfd);
	    for (i=0; i<nActive; i++)
		beasy_closesocket(bfdActive[i]);
	    if (bfdListen != BFD_INVALID_SOCKET)
	...
	else
	{
		...
		if ((nActive + 3) >= FD_SETSIZE)
		{
		    ...
		    {
			err_printf("Critical error: Unable to create a socket.\n");fflush(stdout);
			break;
		    }
		    hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)RedirectIOThread2, (LPVOID)temp_bfd, 0, &dwThreadId);
		    if (hThread == NULL)
		    {
			err_printf("Critical error: Unable to create an io thread\n");fflush(stdout);
			break;
		    }
		    CloseHandle(hThread);
		...
		else
		{
			...
			err_printf("RedirectIOThread2: baccept failed: %d\n", error);fflush(stdout);
			break;
		    }
		...
		for (i=0; n > 0; i++)
		{
			...
			if (num_read == SOCKET_ERROR || num_read == 0)
			{
			    ...
			    i--;
			    /*dbg_printf("(-%d)", nActive);fflush(stdout);*/
			}
			else
			{
			    nDatalen = *(int*)pTemp;
...
void RedirectIOThread(RedirectIOArg *pArg)
{
    ...
    {
	int error = WSAGetLastError();
	err_printf("RedirectIOThread: beasy_create listen socket failed: error %d\n", error);fflush(stdout);
	bsocket_finalize();
	ExitProcess(error);
    }
    ...
    {
	int error = WSAGetLastError();
	err_printf("beasy_create(m_bfdStopIOSignalSocket) failed, error %d\n", error);fflush(stdout);
	ExitProcess(error);
    }
    if (beasy_connect(*pArg->m_pbfdStopIOSignalSocket, g_pszIOHost, g_nIOPort) == SOCKET_ERROR)
    {
	int error = WSAGetLastError();
	err_printf("beasy_connect(m_bfdStopIOSignalSocket, %s, %d) failed, error %d\n", g_pszIOHost, g_nIOPort, error);fflush(stdout);
	ExitProcess(error);
    }
	...
	if (n == SOCKET_ERROR)
	{
	    err_printf("RedirectIOThread: bselect failed, error %d\n", WSAGetLastError());fflush(stdout);
	    break;
	}
	if (n == 0)
	{
	    dbg_printf("RedirectIOThread: bselect returned zero sockets available\n");fflush(stdout);
	    break;
	}
	else
	{
		...
		else
		{
			...
			dbg_printf("Error: redirect IO signal socket closed, exiting\n");
		    else
			err_printf("Error: error reading redirect IO signal socket, error %d\n", WSAGetLastError());
		    fflush(stdout);
		    break;
		}
		n--;
		...
		if ((nActive + 3) >= FD_SETSIZE)
		{
		    ...
		    BFD_CLR(bfdListen, &total_set);
		    bfdListen = BFD_INVALID_SOCKET;
		    /*dbg_printf("started new IO redirection thread\n");fflush(stdout);*/
		}
		else
		{
		    client_bfd = beasy_accept(bfdListen);
		    ...
		    {
			int error = WSAGetLastError();
			err_printf("RedirectIOThread: baccept failed: %d\n", error);fflush(stdout);
			break;
		    }
			...
			if (hThread == NULL)
			{
			    err_printf("Critical error: Standard input redirection thread creation failed. error %d\n", GetLastError());fflush(stdout);
			}
			else
			    CloseHandle(hThread);
		    }
			...
			BFD_SET(client_bfd, &total_set);
			nActive++;
			/*dbg_printf("(+%d:%d)", nActive, bget_fd(client_bfd));fflush(stdout);*/
		    }
		}
		n--;
	    }
		...
		for (i=0; n > 0; i++)
		{
			...
			if (num_read == SOCKET_ERROR || num_read == 0)
			{
			    /*dbg_printf("(-%d:%d)", nActive, bget_fd(bfdActive[i]));fflush(stdout);*/
			    BFD_CLR(bfdActive[i], &total_set);
			    beasy_closesocket(bfdActive[i]);
			    nActive--;
			    bfdActive[i] = bfdActive[nActive];
			...
			else
			{
				...
				/*
				if (num_read == SOCKET_ERROR) 
				    err_printf("(err-%d)", nActive);
				else
				    dbg_printf("(-%d)", nActive);
				fflush(stdout);
				*/
			    }
			    else
				...
				if (g_bDoMultiColorOutput)
				{
				    WaitForSingleObject(g_hConsoleOutputMutex, INFINITE);
				    SetConsoleTextAttribute(hOut, aConsoleColorAttribute[nRank%NUM_OUTPUT_COLORS]);
				    //dbg_printf("(%d)", bget_fd(bfdActive[i]));fflush(stdout);
				    if (WriteFile(hOut, pBuffer, num_read, &num_written, NULL))
					FlushFileBuffers(hOut);
				    else
				    {
    ...
    if (child_abort_bfd != BFD_INVALID_SOCKET)
    {
	/*dbg_printf("signalling child threads to shut down\n");fflush(stdout);*/
	beasy_send(child_abort_bfd, "x", 1);
    ...
    {
	/*dbg_printf("closing listen socket\n");fflush(stdout);*/
	beasy_closesocket(bfdListen);
    }
    for (i=0; i<nActive; i++)
    {
	/*dbg_printf("closing io socket %d\n", i);fflush(stdout);*/
	beasy_closesocket(bfdActive[i]);
    }
    /*dbg_printf("closing signal socket\n");fflush(stdout);*/
    beasy_closesocket(signal_bfd);
    if (hChildThread != NULL)
	CloseHandle(hChildThread);
    /*dbg_printf("RedirectIOThread exiting\n");fflush(stdout);*/
}
