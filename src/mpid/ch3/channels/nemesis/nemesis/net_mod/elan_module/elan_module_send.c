...
{   
   ...
   /*   
    ...
    (MPID_NEM_PACKET_LEN(((MPID_nem_pkt_t *)MPID_NEM_CELL_TO_PACKET (cell)))),MPID_NEM_CELL_SEQN(cell));
    #endif
  */ 
	...
	if (!MPID_nem_elan_event_queue_empty(MPID_nem_module_elan_free_event_queue))
	...
	else
	  {
	     ...
	     if (elan_event_cell->to_proceed)
		  ...
		  /*		  
		   #ifdef TRACE
		   fprintf(stdout,"[%i] ==== ELAN SEND : Direct Pending FREE Q Event is Empty , creating event for cell %p !!!==== \n", MPID_nem_mem_region.rank,elan_event_cell);
		   #endif
		   */ 
