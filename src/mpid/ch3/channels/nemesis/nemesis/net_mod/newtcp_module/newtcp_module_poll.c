...
/*     printf ("breakout\n");//DARIUS */
    ...
    if (len < MPID_NEM_MIN_PACKET_LEN || len < MPID_NEM_PACKET_LEN (MPID_NEM_CELL_TO_PACKET (cell)))
    {
        ...
        MPIU_Assert (VC_FIELD(vc, pending_recv).cell == NULL);
        VC_FIELD(vc, pending_recv).cell = cell;
        VC_FIELD(vc, pending_recv).end = (char *)MPID_NEM_CELL_TO_PACKET (cell) + len;
        VC_FIELD(vc, pending_recv).len = len;
        MPIU_Assert (VC_FIELD(vc, pending_recv).len < MPID_NEM_MAX_PACKET_LEN);
        /* {//DARIUS */
...
static int receive_exactly_one_packet (MPIDI_VC_t *vc)
{
    ...
    if (VC_FIELD(vc, pending_recv).len < MPID_NEM_MIN_PACKET_LEN)
    {
        CHECK_EINTR (bytes_recvd, recv (VC_FIELD(vc, sc)->fd, VC_FIELD(vc, pending_recv).end, MPID_NEM_MIN_PACKET_LEN - VC_FIELD(vc, pending_recv).len, 0));
        if (bytes_recvd <= 0)
        {
...
/*             printf("  read  %d (%u)\n", bytes_recvd, *((char *)VC_FIELD(vc, pending_recv).end));//DARIUS */
    ...
    MPIU_Assert (MPID_NEM_PACKET_LEN (MPID_NEM_CELL_TO_PACKET (VC_FIELD(vc, pending_recv).cell)) <= MPID_NEM_MAX_PACKET_LEN);

    /* try to receive the rest of the packet */
    if (VC_FIELD(vc, pending_recv).len < MPID_NEM_PACKET_LEN (MPID_NEM_CELL_TO_PACKET (VC_FIELD(vc, pending_recv).cell)))
    {
        CHECK_EINTR (bytes_recvd, recv (VC_FIELD(vc, sc)->fd, VC_FIELD(vc, pending_recv).end,
                                        MPID_NEM_PACKET_LEN (MPID_NEM_CELL_TO_PACKET (VC_FIELD(vc, pending_recv).cell)) -
...
/*             printf("  read  %d (%d)\n", bytes_recvd, *((char *)VC_FIELD(vc, pending_recv).end));//DARIUS */
    ...
    MPID_nem_queue_enqueue (MPID_nem_process_recv_queue, VC_FIELD(vc, pending_recv).cell);
/*     printf ("enqueue1 (%d)\n", VC_FIELD(vc, pending_recv).cell->pkt.header.seqno);//DARIUS */
   VC_FIELD(vc, pending_recv).cell = NULL;
...
int MPID_nem_newtcp_module_recv_handler (struct pollfd *pfd, sockconn_t *sc)
{
    ...
    if (VC_FIELD(vc, pending_recv).cell)
    {
...
/*             printf("read %d into tmp_cell (%d)\n", bytes_recvd, *((char *)VC_FIELD(vc, pending_recv).end));//DARIUS */
        ...
        if (VC_FIELD(vc, pending_recv).len >= MPID_NEM_MIN_PACKET_LEN)
        {
           ...
           /* fast path: single packet case */
            ...
            {
                MPID_nem_queue_enqueue (MPID_nem_process_recv_queue, VC_FIELD(vc, pending_recv).cell);
/*                 printf ("enqueue2 %d (%d)\n", VC_FIELD(vc, pending_recv).len, VC_FIELD(vc, pending_recv).cell->pkt.header.seqno);//DARIUS */
                VC_FIELD(vc, pending_recv).cell = NULL;
                /*printf (" recvd whole pkt\n");*/
                MPIU_Assert (VC_FIELD(vc, pending_recv).cell == NULL || VC_FIELD(vc, pending_recv).len < MPID_NEM_MAX_PACKET_LEN);
              goto fn_exit;
            }
            else if (VC_FIELD(vc, pending_recv).len > MPID_NEM_PACKET_LEN (MPID_NEM_CELL_TO_PACKET (VC_FIELD(vc, pending_recv).cell)))
            {
                /* received more than one packet */
                mpi_errno = breakout_pkts (vc, VC_FIELD(vc, pending_recv).cell, VC_FIELD(vc, pending_recv).len);
                if (mpi_errno) MPIU_ERR_POP (mpi_errno);
                MPIU_Assert (VC_FIELD(vc, pending_recv).cell == NULL || VC_FIELD(vc, pending_recv).len < MPID_NEM_MAX_PACKET_LEN);
           }
        }
    ...
    if (recv_overflow_buf.start)
    {
        while (!MPID_nem_queue_empty (MPID_nem_newtcp_module_free_queue))
        {
            ...
            if (len < MPID_NEM_MIN_PACKET_LEN || len < MPID_NEM_PACKET_LEN (pkt))
            {
                ...
                MPIU_Assert (VC_FIELD(recv_overflow_buf.vc, pending_recv).cell == NULL);
                MPIU_Assert (len < MPID_NEM_MIN_PACKET_LEN || MPID_NEM_PACKET_LEN (MPID_NEM_CELL_TO_PACKET (cell)) <= MPID_NEM_MAX_PACKET_LEN);
                VC_FIELD(recv_overflow_buf.vc, pending_recv).cell = cell;
                VC_FIELD(recv_overflow_buf.vc, pending_recv).end = (char *)(&cell->pkt) + len;
                VC_FIELD(recv_overflow_buf.vc, pending_recv).len = len;
                MPIU_Assert (VC_FIELD(recv_overflow_buf.vc, pending_recv).len < MPID_NEM_MAX_PACKET_LEN);
