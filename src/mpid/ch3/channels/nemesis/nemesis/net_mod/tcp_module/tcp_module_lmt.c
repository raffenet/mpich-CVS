...
static int create_r_cookie (char *hostname, int port, int data_sz, char **cookie, unsigned int *len);
...
int MPID_nem_tcp_module_lmt_start_recv (MPIDI_VC_t *vc, MPID_Request *req)
{
    ...
    else if (data_sz < vc->ch.net.tcp.lmt_s_len)
    {
        /* message will be truncated */
        r_len = data_sz;
  	req->status.MPI_ERROR = MPIR_Err_create_code (MPI_SUCCESS, MPIR_ERR_RECOVERABLE, FCNAME, __LINE__, MPI_ERR_TRUNCATE, "**truncate", "**truncate %d %d", vc->ch.net.tcp.lmt_s_len, r_len);
    }
