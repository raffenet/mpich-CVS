    ...
    for (i=0; i<nProc; i++)
    {
		...
		while (pSharedProcess[i].nRank != i)
            ...
            if (pg->pSharedProcessHandles[i] == NULL)
            {
                ...
                mpi_errno = MPIR_Err_create_code(MPI_ERR_OTHER, "**OpenProcess", "**OpenProcess %d %d", i, err); /*"unable to open process %d, error %d\n", i, err);*/
            }
#else
	    ...
	    {
                mpi_errno = MPIR_Err_create_code(MPI_ERR_OTHER, "**open", "**open %s %d", filename, pSharedProcess[i].nPid); /*"failed to open mem file, '%s', for process %d\n", filename, pSharedProcess[i].nPid);*/
	    }
#endif
        }
        else
...
void *MPIDI_CH3I_SHM_Get_mem(MPIDI_CH3I_Process_group_t *pg, int nTotalSize, int nRank, int nNproc, BOOL bUseShm)
{
	...
	if (pg->id == -1) 
	{
	    mpi_errno = MPIR_Err_create_code(MPI_ERR_OTHER, "**shmget", "**shmget %d", errno); /*"Error in shmget\n");*/
	    MPIDI_FUNC_EXIT(MPID_STATE_MPIDI_CH3I_SHM_GET_MEM);
	    return mpi_errno;
	}
	...
	if (pg->id == NULL) 
	{
	    mpi_errno = MPIR_Err_create_code(MPI_ERR_OTHER, "**CreateFileMapping", "**CreateFileMapping %d", GetLastError()); /*"Error in CreateFileMapping, %d\n", GetLastError());*/
	    MPIDI_FUNC_EXIT(MPID_STATE_MPIDI_CH3I_SHM_GET_MEM);
	    return mpi_errno;
	}
#else
#error *** No shared memory allocation function specified ***
	...
	if (pg->addr == (void*)-1)
	{
	    mpi_errno = MPIR_Err_create_code(MPI_ERR_OTHER, "**shmat", "**shmat %d", errno); /*"Error from shmat %d\n", errno);*/
	    MPIDI_FUNC_EXIT(MPID_STATE_MPIDI_CH3I_SHM_GET_MEM);
	    return NULL;
	}
