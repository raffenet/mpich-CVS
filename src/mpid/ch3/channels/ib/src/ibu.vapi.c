...
int ibu_rdma_write(ibu_t ibu, void *sbuf, void *rbuf, int len)
{
    /* pin sbuf */
    /* post rdma sends */
    return IBU_SUCCESS;
...
int ibu_rdma_read(ibu_t ibu, void *rbuf, void *sbuf, int len)
{
    /* pin rbuf */
    /* post rdma reads */
    return IBU_SUCCESS;
...
int MPIDI_CH3I_rdma_writev(MPIDI_VC *vc, MPID_Request *sreq)
{
	...
	*/
	...
	for (i=sreq->ch.iov_offset; i<send_count; i++)
	{
		...
		ibu_rdma_write(vc->ch.ibu, rbuf, sbuf, len);
...
int MPIDI_CH3I_rdma_readv(MPIDI_VC *vc, MPID_Request *rreq)
{
	...
	*/
	...
	for (i=rreq->ch.iov_offset; i<recv_count; i++)
	{
		...
		ibu_rdma_read(vc->ch.ibu, rbuf, sbuf, len);
