...
int MPIDI_CH3_iStartMsgv(MPIDI_VC_t * vc, MPID_IOV * iov, int n_iov, MPID_Request ** sreq_ptr)
{
    ...
    stream_no = Req_Stream_from_pkt_and_req(pkt, *sreq_ptr); /* FIXME could segfault...*/
    ppid = 0;
	...
	if (!SEND_ACTIVE(vc, stream_no)) /* MT */
	{
		...
		while (offset < n_iov)
		{
			...
			/* --END ERROR HANDLING-- */
			//MPIDI_CH3I_SendQ_enqueue_head(vc, sreq);  /* FIXME brad : do we want to enqueue AND make active? */
			...
			if (mpi_errno != MPI_SUCCESS)
			{
			    mpi_errno = MPIR_Err_create_code(mpi_errno, MPIR_ERR_FATAL, FCNAME, __LINE__, MPI_ERR_OTHER,
							     "**ch3|sock|postwrite", "ch3|sock|postwrite %p %p %p",  /* TODO : add new error code */
							     sreq, vc->ch, vc);
			}
			/* --END ERROR HANDLING-- */
			break;
