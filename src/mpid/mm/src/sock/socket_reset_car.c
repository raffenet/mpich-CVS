...
int socket_reset_car(MM_Car *car_ptr)
{
    ...
    MPIDI_STATE_DECL(MPID_STATE_SOCKET_RESET_CAR);

    MPIDI_FUNC_ENTER(MPID_STATE_SOCKET_RESET_CAR);

    buf_ptr = car_ptr->buf_ptr;
    if (buf_ptr == NULL)
    {
	MPIDI_FUNC_EXIT(MPID_STATE_SOCKET_RESET_CAR);
	return -1;
    }
	...
	car_ptr->data.socket.buf.simple.num_written = 0;
	break;
    case MM_TMP_BUFFER:
	car_ptr->data.socket.buf.tmp.num_written = 0;
	break;
    case MM_VEC_BUFFER:
	if (car_ptr->type & MM_WRITE_CAR)
	{
	    ...
	    car_ptr->data.socket.buf.vec_write.total_num_written = 0;
	...
	else
	{
	    ...
	    car_ptr->data.socket.buf.vec_read.total_num_read = 0;
...
/*@
   socket_setup_packet_car - set up a car to read or write an mpid packet
...
{
    MM_Segment_buffer *buf_ptr;
    MPIDI_STATE_DECL(MPID_STATE_SOCKET_SETUP_PACKET_CAR);

    MPIDI_FUNC_ENTER(MPID_STATE_SOCKET_SETUP_PACKET_CAR);
	...
	car_ptr->data.socket.buf.simple.num_written = -1;
#endif
	...
	break;
    default:
	err_printf("Error: socket_setup_packet_car: invalid car type, %d\n", read_write);
	break;
    }
...
int socket_setup_packet_car(MPIDI_VC *vc_ptr, MM_CAR_TYPE read_write, int src_dest, MM_Car *car_ptr)
{
	...
	car_ptr->data.socket.buf.vec_read.cur_num_read = 0;
	car_ptr->data.socket.buf.vec_read.total_num_read = 0;
	car_ptr->data.socket.buf.vec_read.vec[0].MPID_IOV_BUF = (void*)&car_ptr->msg_header.pkt;
	car_ptr->data.socket.buf.vec_read.vec[0].MPID_IOV_LEN = sizeof(MPID_Packet);
	car_ptr->data.socket.buf.vec_read.vec_size = 1;
	...
	car_ptr->data.socket.buf.vec_write.total_num_written = 0;
	car_ptr->data.socket.buf.vec_write.num_read_copy = 0;
	car_ptr->data.socket.buf.vec_write.vec[0].MPID_IOV_BUF = (void*)&car_ptr->msg_header.pkt;
	car_ptr->data.socket.buf.vec_write.vec[0].MPID_IOV_LEN = sizeof(MPID_Packet);
	car_ptr->data.socket.buf.vec_write.vec_size = 1;
