...
int MPID_Init(int * argc, char *** argv, int requested, int * provided, int * has_args, int * has_env)
{
    ...
    MPID_Comm * comm;
    int n;
    const mpig_cm_vtable_t * cm_vtable;
    globus_result_t grc;
    bool_t failed;
...
#       endif
    ...
    for (n = 0; ; n++)
    {
	cm_vtable = mpig_cm_vtables[n];
	if (cm_vtable == NULL) break;
	cm_vtable->init(argc, argv, &mpi_errno, &failed);
	MPIU_ERR_CHKANDJUMP1((failed), mpi_errno, MPI_ERR_OTHER, "**globus|cm_init", "**globus|cm_init %s", cm_vtable->name);
    }
	...
	cm_vtable = mpig_cm_vtables[n];
	if (cm_vtable == NULL) break;
	if (cm_vtable->add_contact_info != NULL)
	{
	    cm_vtable->add_contact_info(&bc, &mpi_errno, &failed);
...
#	    endif
		...
		for (n = 0; ; n++)
		{
		    cm_vtable = mpig_cm_vtables[n];
		    if (cm_vtable == NULL) break;
		    if (cm_vtable->extract_contact_info != NULL)
		    {
			cm_vtable->extract_contact_info(vc, &mpi_errno, &failed);
			MPIU_ERR_CHKANDSTMT1((failed), mpi_errno, MPI_ERR_OTHER, {goto vc_unlock;},
			    "**globus|cm_extract_contact", "**globus|cm_extract_contact %s", cm_vtable->name);
		    }
		}
		...
		for (n = 0; ; n++)
		{
		    cm_vtable = mpig_cm_vtables[n];
		    if (cm_vtable == NULL) break;
		    if (cm_vtable->select_module != NULL)
		    {
			cm_vtable->select_module(vc, &selected, &mpi_errno, &failed);
			MPIU_ERR_CHKANDSTMT3((failed), mpi_errno, MPI_ERR_OTHER, {goto vc_unlock;},
			    "**globus|cm_select_module","**globus|cm_select_module %s %s %d", cm_vtable->name, pg_id, pg_rank);
			if (selected) break;
		    }
		}
...
int MPID_Finalize()
{
    ...
    /* shutdown the communication modules.  each module is responsible for insuring that any VCs managed by it are disconnected
       before returning. */
    for (n = sizeof(mpig_cm_vtables) / sizeof(mpig_cm_vtable_t *) - 2; n >= 0; n--)
    {
