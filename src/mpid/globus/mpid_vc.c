...
typedef struct mpig_vcrt_serialize_pg_table_entry
{
    mpig_pg_t * pg;
    int index;
}
mpig_vcrt_serialize_pg_table_entry_t;
...
int mpig_vcrt_serialize_object(mpig_vcrt_t * const vcrt, char ** const str_p)
{
    ...
    globus_hashtable_t pg_table;
    bool_t pg_table_created = FALSE;
    int pg_count;
    mpig_vcrt_serialize_pg_table_entry_t * pg_table_entry;
    char * bc_str = NULL;
    ...
    rc = globus_hashtable_init(&pg_table, 13, globus_hashtable_voidp_hash, globus_hashtable_voidp_keyeq);
    if (rc != GLOBUS_SUCCESS)
    {   /* --BEGIN ERROR HANDLING-- */
	MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_ERROR | MPIG_DEBUG_LEVEL_COMM, "ERROR: init of PG hash table failed vcrt="
	    MPIG_PTR_FMT, (MPIG_PTR_CAST) vcrt));
	MPIU_ERR_SET1(mpi_errno, MPI_ERR_OTHER, "**nomem", "**nomem %s", "PG hash table");
	goto fn_fail;
    }   /* --END ERROR HANDLING-- */
    pg_table_created = TRUE;
	...
	for (p = 0; p < vcrt->size; p++)
	{
	    ...
	    pg_table_entry = globus_hashtable_lookup(&pg_table, (void *) pg);
	    ...
	    {
		pg_table_entry = (mpig_vcrt_serialize_pg_table_entry_t *)
		    MPIU_Malloc(sizeof(mpig_vcrt_serialize_pg_table_entry_t));
		if (pg_table_entry == NULL)
		{   /* --BEGIN ERROR HANDLING-- */
		    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_ERROR | MPIG_DEBUG_LEVEL_COMM, "ERROR: allocation of a PG table entry "
			"failed: vcrt=" MPIG_PTR_FMT ", pg_count=%d", (MPIG_PTR_CAST) vcrt, pg_count));
		    MPIU_ERR_SET1(mpi_errno, MPI_ERR_OTHER, "**nomem", "**nomem %s", "PG table entry");
		    goto fn_fail;
		}   /* --END ERROR HANDLING-- */

		pg_table_entry->pg = pg;
		pg_table_entry->index = pg_count;
		
		rc = globus_hashtable_insert(&pg_table, (void *) pg, (void *) pg_table_entry);
		if (rc != GLOBUS_SUCCESS)
		{   /* --BEGIN ERROR HANDLING-- */
		    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_ERROR | MPIG_DEBUG_LEVEL_COMM, "ERROR: insert into PG hash table failed: "
			"vcrt=" MPIG_PTR_FMT ", pg=" MPIG_PTR_FMT, (MPIG_PTR_CAST) vcrt, (MPIG_PTR_CAST) pg));
		    MPIU_ERR_SET1(mpi_errno, MPI_ERR_OTHER, "**nomem", "**nomem %s", "PG hash table entry");
		    goto fn_fail;
		}   /* --END ERROR HANDLING-- */
    ...
    pg_table_entry = (mpig_vcrt_serialize_pg_table_entry_t *) globus_hashtable_first(&pg_table);
	...
	globus_hashtable_remove(&pg_table, (void *) pg_table_entry);
	...
	pg_table_entry = (mpig_vcrt_serialize_pg_table_entry_t *) globus_hashtable_next(&pg_table);
    }
  ...
  fn_return:
    ...
    if (pg_table_created) globus_hashtable_destroy(&pg_table);
...
int mpig_vc_construct_contact_info(mpig_vc_t * vc)
{
    ...
    mpi_errno = mpig_bc_get_contact(bc, "GLOBUS_LAN_ID", &lan_id, &found);
    if (mpi_errno)
    {   /* --BEGIN ERROR HANDLING-- */
