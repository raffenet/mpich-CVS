...
MPIG_STATIC int mpig_topology_get_vc_match(mpig_vc_t * vc1, mpig_vc_t * vc2, int level, bool_t * match);
...
int mpig_topology_comm_construct(MPID_Comm * const comm)
{
    ...
    /* topology information is on applicable to intracommunicators */
...
int mpig_topology_comm_destruct(MPID_Comm * const comm)
{
    ...
    /* XXX: delete the attribute keys from the communicator if the keys are still attached to the communicator (the user could
       have deleted them).  the COPY of topology information will be destroyed by the destructor callbacks */

    /* XXX: destroy the topology information */
...
MPIG_STATIC int mpig_topology_get_vc_match(mpig_vc_t * vc1, mpig_vc_t * vc2, int level, bool_t * match_p)
{
    const char fcname[] = MPIG_QUOTE(FUNCNAME);
    int n;
    unsigned levels_out;
    ...
    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_FUNC | MPIG_DEBUG_LEVEL_COMM, "entering: vc1=" MPIG_PTR_FMT ", vc2=" MPIG_PTR_FMT
	", level=%d", (MPIG_PTR_CAST) vc1, (MPIG_PTR_CAST) vc2, level));

    if ((level & mpig_vc_get_topology_levels(vc1) & mpig_vc_get_topology_levels(vc2)) == 0) goto fn_return;
  ...
  fn_return:
    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_FUNC | MPIG_DEBUG_LEVEL_COMM, "exiting: vc1=" MPIG_PTR_FMT ", vc2=" MPIG_PTR_FMT
...
MPIG_STATIC int mpig_topology_destroy_depths_attr(MPI_Comm comm, int keyval, void * attr, void * state)
{
    ...
    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_FUNC | MPIG_DEBUG_LEVEL_COMM, "entering: comm=" MPIG_HANDLE_FMT ", keyval=" MPIG_HANDLE_FMT
	", depths=" MPIG_PTR_FMT, comm, keyval, (MPIG_PTR_CAST) keyval));

    MPIU_Free(depths);
    
    /* fn_return: */
    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_FUNC | MPIG_DEBUG_LEVEL_COMM, "exiting: comm=" MPIG_HANDLE_FMT ", keyval=" MPIG_HANDLE_FMT
	", depths=" MPIG_PTR_FMT, comm, keyval, (MPIG_PTR_CAST) keyval));
...
MPIG_STATIC int mpig_topology_destroy_colors_attr(MPI_Comm comm, int keyval, void * attr, void * state)
{
    ...
    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_FUNC | MPIG_DEBUG_LEVEL_COMM, "entering: comm=" MPIG_HANDLE_FMT ", keyval=" MPIG_HANDLE_FMT
	", colors=" MPIG_PTR_FMT, comm, keyval, (MPIG_PTR_CAST) keyval));

    MPIR_Nest_incr();

    mpi_errno = NMPI_Comm_size(comm, &size);
    MPIU_ERR_CHKANDJUMP((mpi_errno), mpi_errno, MPI_ERR_OTHER, "**globus|mpi_comm_size");
