...
static void mpig_cm_xio_vc_destruct_fn(mpig_vc_t * vc);
static const char * mpig_cm_xio_vc_state_get_string(mpig_cm_xio_vc_state_t vc_state);
...
static int mpig_cm_xio_adi3_cancel_recv(MPID_Request * rreq);

static int mpig_cm_xio_adi3_cancel_send(MPID_Request * sreq);
...
{
    static const char fcname[] = MPIG_QUOTE(FUNCNAME);
    const int send_ctx = comm->context_id + ctxoff;
	...
	rank, tag, MPIG_PTR_CAST(comm), send_ctx));

    MPIU_Assert(mpig_vc_get_cm_module_type(vc) == MPIG_CM_TYPE_XIO);

    mpig_request_create_isreq(MPIG_REQUEST_TYPE_SEND, 2, 1, (void *) buf, cnt, dt, rank, tag, send_ctx, comm, vc, &sreq);
    ...
    const int rank, const int tag, MPID_Comm * const comm, const int ctxoff, MPID_Request ** const sreqp)
{
    ...
    mpig_request_create_isreq(MPIG_REQUEST_TYPE_RSEND, 2, 1, (void *) buf, cnt, dt, rank, tag, send_ctx, comm, vc, &sreq);
    mpig_cm_xio_request_construct(sreq);
    mpig_cm_xio_request_set_cc(sreq, 1);
    ...
    const int rank, const int tag, MPID_Comm * const comm, const int ctxoff, MPID_Request ** const sreqp)
{
    ...
    mpig_request_create_isreq(MPIG_REQUEST_TYPE_SSEND, 2, 1, (void *) buf, cnt, dt, rank, tag, send_ctx, comm, vc, &sreq);
    mpig_cm_xio_request_construct(sreq);
    ...
    const int rank, const int tag, MPID_Comm * const comm, const int ctxoff, MPID_Request ** const rreqp)
{
    ...
    mpi_errno = mpig_recvq_deq_unexp_or_enq_posted(rank, tag, recv_ctx, NULL, &rreq_found, &rreq);
    if (mpi_errno)
    {   /* --BEGIN ERROR HANDLING-- */
    ...
    /* inform the PE of the start of an operation so that this CM is polled.  mpig_cm_xio_vc_recv_unexpected() will reduce the
       count if the request has already completed, so we must do this first to avoid a negative count. */
    mpig_cm_xio_pe_start_op();
	...
	mpig_request_construct_irreq(rreq, 2, 1, buf, cnt, dt, rank, tag, recv_ctx, comm, vc);
    ...
    const int rank, const int tag, MPID_Comm * const comm, const int ctxoff, MPI_Status * const status)
{
  ...
  fn_return:
	...
	MPIG_PTR_FMT ", recv_ctx=%d, mpi_errno=" MPIG_ERRNO_FMT, rank, tag, MPIG_PTR_CAST(comm), recv_ctx, mpi_errno));
    ...
    const int rank, const int tag, MPID_Comm * const comm, const int ctxoff, int * const found_p, MPI_Status * const status)
{
  ...
  fn_return:
    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_FUNC | MPIG_DEBUG_LEVEL_ADI3 | MPIG_DEBUG_LEVEL_PT2PT, "exiting: rank=%d, tag=%d, comm="
	MPIG_PTR_FMT ", recv_ctx=%d, found=%s, mpi_errno=" MPIG_ERRNO_FMT, rank, tag, MPIG_PTR_CAST(comm), recv_ctx,
        MPIG_BOOL_STR(found), mpi_errno));
...
int mpig_cm_xio_adi3_cancel_recv(MPID_Request * rreq)
{
    ...
    int mpi_errno = MPI_SUCCESS;
    MPIG_STATE_DECL(MPID_STATE_mpig_cm_xio_adi3_cancel_recv);

    MPIG_UNUSED_VAR(fcname);

    MPIG_FUNC_ENTER(MPID_STATE_mpig_cm_xio_adi3_cancel_recv);
    ...
    if (cancelled)
    {
        ...
        /* inform the PE that the receive operation has completed */
        mpig_cm_xio_pe_end_op(FALSE);
    ...
    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_FUNC | MPIG_DEBUG_LEVEL_ADI3 | MPIG_DEBUG_LEVEL_PT2PT, "exiting: rreq=" MPIG_HANDLE_FMT
    ", rreqp=" MPIG_PTR_FMT, rreq->handle, MPIG_PTR_CAST(rreq)));
    MPIG_FUNC_EXIT(MPID_STATE_mpig_cm_xio_adi3_cancel_recv);
    return mpi_errno;
}
/* mpig_cm_xio_adi3_cancel_recv() */
...
static int mpig_cm_xio_adi3_cancel_send(MPID_Request * const sreq)
{
	...
	sreq_locked = TRUE;
	{
	    ...
	    {
		mpig_request_get_envelope(sreq, &rank, &tag, &send_ctx);
		...
		MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_PT2PT, "sending cancel request to remote process: vc=" MPIG_PTR_FMT ", sreq="
		    MPIG_HANDLE_FMT ", sreqp=" MPIG_PTR_FMT ", rank=%d, tag=%d, send_ctx=%d", MPIG_PTR_CAST(vc), sreq->handle,
...
static int mpig_cm_xio_vc_recv_unexpected(mpig_vc_t * const vc, MPID_Request * const rreq)
{
	...
	else if (mpig_cm_xio_request_get_state(rreq) == MPIG_CM_XIO_REQ_STATE_RECV_DRAIN_DATA)
	{
	    /* an error occurred necessitating that the data be drained from the network, and that data is still being drained.
	       the completion counter by the routine that initiates the draining of the data.  when the drain completes, the
	       count is decremented.  the counter is decremented here so that when both operations complete the request will
	       complete without an futher explicit coordination. */
	    mpig_cm_xio_request_dec_cc(rreq, &rreq_complete);
	}
    }
