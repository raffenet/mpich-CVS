 ...
 */
static int mpig_pm_ws_init(mpig_pm_t * pm, bool_t * my_pm_p);
...
static void globus_rz_data_callback(globus_result_t res, const char *data, size_t length, void *args);
 ...
 * mpig_pm_ws_init()
 ********************/ 
static int mpig_pm_ws_init(mpig_pm_t * const pm, bool_t * my_pm_p)
{       
    globus_result_t res;
...
static void mpig_pm_ws_mpig_pm_ws_gather_subjob_data(struct rz_data *subjob_data,
			    char *my_checkinbuff, 
			    int hdr_len, 
			    int my_checkinbufflen)
{
...
#else /* !defined(MPIG_GLOBUS_RENDEZVOUS_INSTALLED) */
...
#endif /* #if/else defined(MPIG_GLOBUS_RENDEZVOUS_INSTALLED) */
