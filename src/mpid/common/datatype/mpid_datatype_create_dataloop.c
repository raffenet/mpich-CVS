...
void MPID_Datatype_create_dataloop(MPI_Datatype type,
				   MPID_Dataloop **dlp_p,
				   int *dlsz_p,
				   int *dldepth_p,
				   int flags)
{
    ...
    MPID_Dataloop *old_dlp;
    int old_dlsz, old_dldepth, recursed = 0;
	...
	*dldepth_p = 0;
	...
	if (old_dtp->dataloop == NULL)
	{
	    ...
	    MPID_Datatype_create_dataloop(types[0],
					  &old_dlp,
					  &old_dlsz,
					  &old_dldepth,
					  flags);
	...
	case MPI_COMBINER_DUP:
		...
		*dldepth_p = old_dldepth;
		...
		MPID_Dataloop_create_contiguous(1,
						types[0], 
	...
	case MPI_COMBINER_STRUCT_INTEGER:
	    for (i = 1; i < ints[0]; i++) {
		int type_combiner;
		MPID_Type_get_envelope(types[i], &dummy1, &dummy2, &dummy3,
				       &type_combiner);
		...
		if (type_combiner != MPI_COMBINER_NAMED) {
			...
			MPID_Datatype_create_dataloop(types[i],
						      &old_dlp,
						      &old_dlsz,
						      &old_dldepth,
						      flags);
	...
	case MPI_COMBINER_SUBARRAY:
	    ndims = ints[0];
	    MPID_Type_convert_subarray(ndims,
				       &ints[1] /* sizes */,
	    ...
	    MPID_Datatype_create_dataloop(tmptype,
					  dlp_p,
					  dlsz_p,
					  dldepth_p,
					  flags);
	...
	case MPI_COMBINER_DARRAY:
	    ndims = ints[2];
	    MPID_Type_convert_darray(ints[0] /* size */,
				     ints[1] /* rank */,
				     ndims,
				     &ints[3] /* gsizes */,
