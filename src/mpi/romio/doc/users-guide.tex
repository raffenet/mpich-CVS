...
open'' is used.  When set to \textt{true}, ROMIO will make an effort to avoid
...
applicable to all file system types: 
\begin{itemize}
\item \textt{no\_indep\_rw} -- If the application plans on performing only 
   collecitve operations and this hint is set to ``true'', then ROMIO can
   have just the aggregators open a file.   The \textt{cb\_config\_list} and
   \text{cb\_nodes} hints can be given to further control which nodes are
   aggregators.  
\end{itemize}
