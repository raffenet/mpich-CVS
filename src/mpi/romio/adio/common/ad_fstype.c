			  ...
			  ADIOI_Fns **ops, int *error_code)
{
...
#ifdef MPICH2
	    *error_code = MPIR_Err_create_code(MPI_SUCCESS, MPIR_ERR_RECOVERABLE, myname, MPI_ERR_IO, "**iofstype", 0);
	    return;
	...
	if (myerrcode != MPI_SUCCESS) {
#ifdef MPICH2
	    *error_code = MPIR_Err_create_code(MPI_SUCCESS, MPIR_ERR_RECOVERABLE, myname, MPI_ERR_IO, "**io", 0);
	    return;
#elif defined(PRINT_ERR_MSG)
