...
int ADIOI_MPIMUTEX_Create(int rank, MPI_Comm comm, mpimutex_t *mutex_p)
{
    ...
    /* atomic read-modify-update not possible with one-sided semantics, but we
     * can write one byte and read all other bytes with a carefully crafted
     * datatype */
	...
	mpi_err = MPI_Win_create(NULL, 0, 1, MPI_INFO_NULL, mutex->comm,
		&mutex->waitlistwin);
    }
    if (mpi_err != MPI_SUCCESS) goto err_return; 
...
/* the create, lock, unlock, free, set and get routines are much like above,
 * but take a slightly different mpimutex type: one with a "payload" of a file
 * offset */
int ADIOI_MPIMUTEX_FP_Create(int rank, MPI_Comm comm, 
		mpimutex_fp_t *mutex_p)
{
    int nprocs, myrank, mpi_err;
    mpimutex_fp_t mutex = NULL;

    int blklens[2];
    MPI_Aint disps[2];

    mutex = malloc(sizeof(struct mpimutex_fp));
    if (!mutex) goto err_return;

    memset(mutex, 0, sizeof(struct mpimutex_fp));
	 ...
	 * uses the "nprocs-1" additional bytes */
