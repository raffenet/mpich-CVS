...
ibp_open(const char *uri, int mode, int m)
{
	...
	// environment setup
        if(getenv("LBONE_SERVER")) {
                handle->lbone_server = getenv("LBONE_SERVER");
        } else {
                handle->lbone_server = LBONE_SERVER;
        }
	...
	if(getenv("LBONE_LOCATION")) {
		handle->lbone_location = getenv("LBONE_LOCATION");
	} else {
		handle->lbone_location = LBONE_LOCATION;
	}
	...
	if(getenv("LORS_SIZE")) {
                handle->lors_size = atoi(getenv("LORS_SIZE"));
        } else {
	...
	if(!handle->lbone_server && !(mode & O_RDONLY || 
				     !mode || mode & O_RDWR)) {
		errno = EINVAL;
		return (void *)-1;
	}
	...
	if(mode & O_WRONLY || mode & O_CREAT || mode & O_RDWR) {
		int fd;
		fd = open(handle->filename, mode, m);
		if(fd == -1) {
			free(handle->buffer);
			free(handle);
			return (void *)-1;
        ...
        /* updated by YING */ 
	if(mode & O_RDONLY || !mode || mode & O_RDWR) {
		ret = lorsFileDeserialize(&handle->ex, handle->filename, NULL);
		if(ret != LORS_SUCCESS && !(mode & O_RDWR)) {
			errno = EIO;
			return (void *)-1;
			...
			LorsEnum list = NULL,it=NULL;
				...
				int size = (1 + 
					   handle->lors_blocksize / mp->e2e_bs)
					   * mp->e2e_bs;
                                free(handle->buffer);
...
xio_open(const char *pathname, int flags, ...)
{
	...
	if(1/*strncmp(pathname, IBP_URI, strlen(IBP_URI)) == 0*/) {
		// IBP uri
		...
		if(_handles[ret_fd]->data == (void *)-1) {
