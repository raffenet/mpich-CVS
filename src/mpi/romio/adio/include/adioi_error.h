...
{if (!(fh)) { \
	error_code = MPIR_Err_create_code(MPI_SUCCESS, MPIR_ERR_RECOVERABLE, myname, MPI_ERR_FILE, "**iofilenull", 0);\
	return MPIR_Err_return_file(MPI_FILE_NULL, myname, error_code);}\
 else if ((fh)->cookie != ADIOI_FILE_COOKIE) { \
    error_code = MPIR_Err_create_code(MPI_SUCCESS, MPIR_ERR_RECOVERABLE, myname, MPI_ERR_FILE, "**iofilecorrupt", 0);\
    return MPIR_Err_return_file(MPI_FILE_NULL, myname, error_code); } }
#else /* MPICH-1 */
#define ADIOI_TEST_FILE_HANDLE(fh, myname) \
{if (!(fh)) { \
