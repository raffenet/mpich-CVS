...
.  int sendcounts[] - send counts
.  int sdispls[] - send displs
.  MPI_Datatype sendtypes[] - send types
.  void *recvbuf - receive buffer
.  int recvcounts[] - receive counts
.  int rdispls[] - receive displs
.  MPI_Datatype recvtypes[] - receive types
...
.N MPI_SUCCESS
@*/
EXPORT_MPI_API int MPI_Alltoallw(void *sendbuf, int sendcounts[], int sdispls[], MPI_Datatype sendtypes[], void *recvbuf, int recvcounts[], int rdispls[], MPI_Datatype recvtypes[], MPI_Comm comm)
{
    static const char FCNAME[] = "MPI_Alltoallw";
    int mpi_errno = MPI_SUCCESS;
    MPID_Comm *comm_ptr = NULL;

    MPID_MPI_FUNC_ENTER(MPID_STATE_MPI_ALLTOALLW);
...
#   ifdef HAVE_ERROR_CHECKING
    {
	    ...
	    /* If comm_ptr is not value, it will be reset to null */
            if (mpi_errno) {
                MPID_MPI_FUNC_EXIT(MPID_STATE_MPI_ALLTOALLW);
