...
int main( int argc, char *argv[] )
{
	...
	if (rank == 0) {
	    r1buf = r2buf = -1;
	    MPI_Irecv( &r1buf, 1, MPI_INT, 0, 0, dupcomm, &r1req );
	    MPI_Irecv( &r2buf, 1, MPI_INT, 0, 0, comm, &r2req );
	    s2buf = 17;
	    MPI_Send( &s2buf, 1, MPI_INT, 0, 0, comm );
	    MPI_Test( &r1req, &flag, &status );
	    if (flag) {
		errs++;
	    ...
	    MPI_Wait( &r1req, &status1 );
	    MPI_Wait( &r2req, &status2 );
	    if (r1buf != s1buf) {
		errs++;
