/******************** d2s_util.c ****************************/
/*
  This program converts a dlog file generated using MPE Logging calls into an 
  slog file.
*/

/*
  a dlog file format:
  divided into chunks of 1024 bytes containing a DLOG block.
...
static int state_id = 1;                /* state id for dlog2slog independent of
					   the dlog state id's.*/
...
/* Forward refs */
static int logEvent( DLOG_HEADER *, DLOG_IOStruct * );
static int logMsgEvent(DLOG_HEADER *headr, int event, int data);
static int handle_extra_state_defs(DLOG_STATE *);
static int writeSLOGInterval(DLOG_HEADER *, int, struct list_elemnt);
static int handleStartEvent(int, DLOG_HEADER *, int, int);
static int addState(int, int, int, char *, char *);
static int replace_state_in_list(int, int, char *, char *);
...
int DLOG_init_state_defs(char *filename)
{
	...
	case DLOG_STATE_TYPE:
	    if ( (findState_strtEvnt(pInput->record.state.endetype) == D2S_ERROR) ||
		(findState_endEvnt(pInput->record.state.startetype) == D2S_ERROR) )
	    {
		if (addState(
		    pInput->record.state.stateid,
		    pInput->record.state.startetype,
		    pInput->record.state.endetype,
		...
		fprintf(stderr,__FILE__":%d: event ids defined for state %s already "
		    ...
		    __LINE__, pInput->record.state.description);
		DLOG_CloseInputStruct(&pInput);
		return D2S_ERROR;
	    }
	    break;
...
/****
     ...
     state definitions.
****/
int DLOG_init_all_mpi_state_defs ( )
{
...
/****
     ...
     it looks for DLOG_RAWEVENT types and then passes it on to the logEvent
...
int DLOG_makeSLOG(char *filename)
{
	...
	{
	case DLOG_EVENT_TYPE:
	    if (logEvent(&pInput->header, pInput) == D2S_ERROR)
	    {
		DLOG_CloseInputStruct(&pInput);
		return D2S_ERROR;
	    }
	...
	case DLOG_SENDRECV_TYPE:
	    ...
	    break;
	case DLOG_STATE_TYPE:
	    if (handle_extra_state_defs(&pInput->record.state) == D2S_ERROR)
	    {
		DLOG_CloseInputStruct(&pInput);
		return D2S_ERROR;
	    }
...
****/
static int logEvent(DLOG_HEADER *headr, DLOG_IOStruct *pInput)
{
    int stat_id;
    int event, data;
    int error = D2S_SUCCESS;
	...
	if ( stat_id == MSG_STATE )
	{
	    ...
	    {
		if ( find_msg_elemnt( stat_id, data, headr->procid, 
		     event, &one ) == D2S_ERROR )
		{
		    error = handleStartEvent( stat_id, headr, data, event );
		}
		else
		{
		    error = writeSLOGInterval( headr, event, one );
		}
	    }
	}
	else
    ...
    else if ( (stat_id = findState_endEvnt(event)) != D2S_ERROR ) 
    {
      /*
	...
	if (    find_elemnt(stat_id, data, headr->procid, &one) == D2S_ERROR ) 
	{
        /*
	  ...
	  start event. Though any other dlog-state having the same problem is 
...
/****
     ...
     begins.
****/
static int handle_extra_state_defs(DLOG_STATE *state)
{
...
****/
static int writeSLOGInterval(DLOG_HEADER *headr, int event,
...
/****
     ...
     list. 
****/
static int handleStartEvent(int stat_id, DLOG_HEADER *headr, int data, int etype)
{
  /*
    SLOG STUFF:
    we are not interested in bebits, cpu_id, thread_id, irec_iaddr
...
****/
static int addState(int stat_id, int strt_id, int end_id, char *colr, char *desc)
{
