\ifx\insingle\undefined
\else
% If we're in the single-page version, promote the sections
\typeout{hi!}
...
\file{test/mpi/util/mtest.c} to help in writing new tests, described
in Section~\ref{sec:test-utilities}.

A test in the MPICH2 test suite must follow these rules:
\begin{enumerate}
...
processes.  In particular, an \code{MPI_Abort} is \emph{not} guaranteed to
...
\item In \file{test/mpi/configure.in}, add the \file{Makefile} for
your new directory to the 
\code{AC_OUTPUT} list (e.g., \code{newdir/Makefile}).
...
They are, in order:
\begin{description}
\item[\code{MTest_Init}]Initialize the MTest routines, including
calling \code{MPI_Init} if necessary.  In addition, the following
environment variables are read:
\begin{description}
\item[\code{MPITEST_DEBUG}]If set, turns on internal debugging output
from the MTest package, such as data on created datatypes and communicators.
\item[\code{MPITEST_VERBOSE}]If set to a positive integer, sets the
...
\begin{description}
\item[\code{MTestGetIntracomm}]Like \code{MTestGetIntracommGeneral},
but always provides a communicator (but never \code{MPI_COMM_NULL}).
\item[\code{MTestGetIntercomm}]Like \code{MTestGetIntracomm}, but
...
\code{MPI_COMM_NULL}.  
\item[\code{MTestGetIntercommName}]Like \code{MTestGetIntracommName},
but returns the name of an intercommunicator.
\item[\code{MTestGetComm}]Like \code{MTestGetIntracomm}, but for both
inter- and intra-communicators.
\item[\code{MTestPrintErrorMsg}]Like \code{MTestPrintError}, but
includes an additional message.
\item[\code{MTestGetWin}]Provides an MPI Window object.  Active and
passive windows are provided.  
\item[\code{MTestGetWinName}]Get the name associated with a window object
\end{description}

\if 1=0
% This is an old section and is commented out to the end of the file.
