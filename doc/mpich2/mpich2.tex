...
\begin{verbatim}
#include <ldap.h>
static ?type? ldap_handle = 0;
if (!ldap_handle) {
    ? how do we get the server name ?
    ? Use ldap_url_parse or ldap_is_ldap_url?
    ldap_handle = ldap_init(?? );
    ldap_bind_s( ldap_handle, ?? );
    /* Or ldap_simple_bind_s or ldap_kerberos_bind_s */
    MPIR_Add_finalize( MPID_Nameserver_finalize &ldap_handle );  
          /* See MPI_Finalize */
    }
/* Synchronous call because MPI_LOOKUP_NAME is blocking */
ldap_search_s( ??? ldap_handle, name, value );
...
\begin{verbatim}
ldap_add_s( ldap_handle, name, value ??? );
\end{verbatim}

\subsubsection{\mpifunc{MPI_UNPUBLISH_NAME}}
\begin{verbatim}
ldap_delete_s( ldap_handle, name, ??? );
\end{verbatim}
