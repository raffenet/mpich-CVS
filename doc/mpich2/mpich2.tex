...
special cases:
\begin{description}
\item{MPID_GROUP_MAP_IDENT} for \code{lpid = lrank}
\item{MPID_GROUP_MAP_AFFINE_UNIT} for \code{lpid = offset + lrank}
\item{MPID_GROUP_MAP_AFFINE} for \code{lpid = offset + lrank * stride}
\item{MPID_GROUP_MAP_GENERAL} for the general case.
\end{description}
...
2.50\footnote{Don't ask.  Ok, \texttt{autoconf} jumped from 2.13 to
  2.50 and completely reorganized (good).  Unfortunately, many of the
  internal macros that are used by our extensions were reused with
  different semantics(!)}.  The top-level configure will only
...
\item[\texttt{MPIU\_usage\_printf}]Print a usage message (informational) for
  the user.  This has the same calling sequence as \code{printf}.
\item[\texttt{MPIU\_error\_printf}]Print an error message for the user.  This
