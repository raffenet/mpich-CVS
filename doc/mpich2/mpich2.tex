...
% to order the building of modules based on dependencies.
  ...
  \item[pmi]Implementations of the Process Manager Interface.  The ``simple''
    ...
    be used to select an alternative implementation, such as the \code{remsh}
    ...
    \begin{description}
          ...
          \item[winmpd]Implementation of \code{pmi} for the Windows MPD
...
% is likely to exist only in or for MPICH.
    \item[pm]Process managers.  The default process manager for UNIX is
      ...
      \cfgoption{--with-pm=dir} configure option.  No MPICH code includes
...
used by both C and C++ compilers must \emph{not} use this comment.
...
\item Add a file \file{src/mpid/ch3/channels/newdev/Makefile.sm} .  This file
  is used to create the \file{Makefile.in} file used to build this device.
...
staying sychronized with MPICH2 development.
