...
\item[src]Source files for the MPI implementation
  ...
  \item[mpi]The implementation of the MPI routines
    ...
    \item[datatype]Datatypes (Section~\ref{sec:datatypes})
  ...
  \item[binding]Language bindings
    ...
    \item[fortran90]
    \end{description}
  \item[env]Commands like \code{mpiexec}
  \item[mpe]
  \end{description}
...
\begin{description}
\item MPI Initialized
\item Parameter values in range, including tag values, ranks, counts.
\item Aliasing of input and output parameters (e.g., inbuf and outbuf of
  \code{MPI_Allreduce}) 
\item MPI objects valid (See opaque object discussion)
\item Output parameter pointers valid.  The macro
  \code{MPID_Pointer_is_invalid(void *p)} returns one for invalid pointers.
  Note that this may be as simple as \code{((p) == 0)} and as complex as
  \code{(long(p)<=0)} or \code{9!(p) || (unsigned long)(p) > STACKLIMIT)}.
\end{description}
...
errors abort?  Recommendation: abort.
...
from zero, the error is returned.  The interface is
...
As a software package, MPICH should not perturb the user's environment.  In
...
(and corresponding \code{MPID_Calloc}, etc.) should be used instead of
\code{malloc}, as described in the ADI-3 manual \cite{adi3man}.  
...
In some routines, there may be multiple memory allocations.  If an error is
...
\section{Per Thread and Per Process Data}
\label{sec:perthread}
...
typedef struct {
    ...
    MPID_Timestamp_t timestamps[MAX_STATES];  /* per thread state timers */    
    } MPICH_PerThread_t;
\end{verbatim}
...
\begin{verbatim}
typedef struct {
    MPID_Thread_key_t thread_key;
    MPID_Thread_id_t  master_thread;
} MPICH_PerProcess_t;
\end{verbatim}
...
access the data without using \code{pthread_getspecific}.
...
Each routine will contain references to 
\begin{verbatim}
...
function name (e.g., \code{MPID_STATE_GETHOSTBYNAME}).
...
int PMPI_Foo( int a ) 
{
    ...
    MPID_BEGIN_ERROR_CHECKS;
        if (a < 0) {
            mpi_errno = MPIR_Err_setmsg( MPIi_ERR_NEGATIVE, a );
        }
    MPID_END_ERROR_CHECKS;

    MPID_TimerStateEnd(MPID_STATE_MPI_FOO);
    MPIR_RETURN( errhandler, mpi_errno );
}
\end{verbatim}
...
Question: How is the above supposed to work?
...
\section{sec:chosing-errhandler}. 
