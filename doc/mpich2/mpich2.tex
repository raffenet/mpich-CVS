...
%
% Algorithm
\newenvironment{algorithm}{\begin{obeylines}\obeyspaces\noindent}{\end{obeylines}}
...
the MPI implementation; thus, there is no easy way for the implementation
...
\begin{verbatim}
void MPIi_Set_err_return(void);
void MPIi_Restore_err(void);
MPID_Errhandler *MPIi_Get_err_handler(ds);
\end{verbatim}
...
sticky upper bound marker, lower bound marker).  Do any other
data structures need multiple flags?
...
\mpiconst{access_style}\index{MPI_Info!access_style}, 
\mpiconst{appnum}\index{MPI_Info!appnum}, 
\mpiconst{arch}\index{MPI_Info!arch}, 
\mpiconst{cb_block_size}\index{MPI_Info!cb_block_size}, 
\mpiconst{cb_buffer_size}\index{MPI_Info!cb_buffer_size}, 
\mpiconst{cb_nodes}\index{MPI_Info!cb_nodes}, 
\mpiconst{chunked}\index{MPI_Info!chunked}, 
\mpiconst{chunked_item}\index{MPI_Info!chunked_item}, 
\mpiconst{chunked_size}\index{MPI_Info!chunked_size}, 
\mpiconst{collective_buffering}\index{MPI_Info!collective_buffering}, 
\mpiconst{external32}\index{MPI_Info!external32}, 
\mpiconst{false}\index{MPI_Info!false}, 
\mpiconst{file}\index{MPI_Info!file}, 
\mpiconst{file_perm}\index{MPI_Info!file_perm}, 
\mpiconst{filename}\index{MPI_Info!filename}, 
\mpiconst{host}\index{MPI_Info!host}, 
\mpiconst{internal}\index{MPI_Info!internal}, 
\mpiconst{io_node_list}\index{MPI_Info!io_node_list}, 
\mpiconst{ip_address}\index{MPI_Info!ip_address}, 
\mpiconst{ip_port}\index{MPI_Info!ip_port}, 
\mpiconst{native}\index{MPI_Info!native}, 
\mpiconst{nb_proc}\index{MPI_Info!nb_proc}, 
\mpiconst{no_locks}\index{MPI_Info!no_locks}, 
\mpiconst{num_io_nodes}\index{MPI_Info!num_io_nodes}, 
\mpiconst{path}\index{MPI_Info!path}, 
\mpiconst{random}\index{MPI_Info!random}, 
\mpiconst{read_mostly}\index{MPI_Info!read_mostly}, 
\mpiconst{read_once}\index{MPI_Info!read_once}, 
\mpiconst{reverse_sequential}\index{MPI_Info!reverse_sequential}, 
\mpiconst{sequential}\index{MPI_Info!sequential}, 
\mpiconst{soft}\index{MPI_Info!soft}, 
\mpiconst{striping_factor}\index{MPI_Info!striping_factor}, 
\mpiconst{striping_unit}\index{MPI_Info!striping_unit}, 
\mpiconst{true}\index{MPI_Info!true}, 
\mpiconst{wdir}\index{MPI_Info!wdir}, 
\mpiconst{write_mostly}\index{MPI_Info!write_mostly}, 
\mpiconst{write_once}.
...
Section~\label{sec:err-handling-nested}).
...
Question: To help catch user errors, should this try no to reuse the
...
Call the MPID routine to free keyvals (not yet defined).  
...
Delete an info object.  Uses an MPID object deleter.
...
All datatypes should call a function to compute the extent because the
...
If a sticky ub exists for the old datatype (datatypes for struct), then
    use the sticky ub and set the sticky ub flag
else
    use the \mpids{MPI_Datatype}{true_ub}
/* Similar for the \mpids{MPI_Datatype}{lb} and
...
routines in Section~\label{sec:runtime-params}.  The default alignment
...
Uses the \mpids{MPI_Datatype}{size} field of the datatype.  Note the
special case of a 
...
\begin{algorithm}
If sizeof datatype is zero, then
    If size of message is zero, return zero
    else return \mpiconst{MPI_UNDEFINED}
Compute sizeof message / sizeof datatype.  
If remainder is zero, then 
    the number of elements is this quotient times the number of element in 
    the datatype
Else if all elements in the datatype are the same size then
    (e.g., an indexed case)
    the number of elements is sizeof(message) / sizeof(one element)
...
Else 
    ...
    Take the remainder from sizeof message / sizeof datatype.
    Execute the dataloop algorithm, but with no data movement, keeping track
    of the basic elements.  Note that this requires a version of the 
    dataloop that preserves the original element sizes.  
    Add this count to elements-per-datatype times the quotient.
\end{algorithm}
...
of \mpifunc{MPI_TYPE_CREATE_HINDEXED}.
...
(\mpids{MPI_Datatype}{attributes}) on this datatype.
...
Like \mpifunc{MPI_UNPACK}, but for ``external32''.
...
\item For group2, if necessary, create a new array containing the pairs
  \code{local process id}, \code{local rank}, sorted by \code{local process
    id}. Call this array \mpids{MPI_Group}{lpid_to_lrank}.
...
\item For each rank in ranks1, find the corresponding \code{local process id}
  using \mpids{MPI_Group}{lrank_to_lpid} and then search for that in
...
\begin{adi3}
  ...
  the values to \code{1} for each element specified by the range, and
  then assemble 
...
See \code{MPI_COMM_SIZE}.  Since this is an unusual operation, should
it just access the \mpids{MPI_Group}{size} field of the
\mpids{MPI_Comm}{remote_group}? 
...
there be a \mpids{MPI_Comm}{local_size} field in the communicator?
...
This could have the following interface:
\begin{verbatim}
...
Note that in a multithreaded MPI implementation, this much watch for the race
condition of
\begin{verbatim}
     Thread 1                    Thread 2
 Check queue, no match found
                                 Handling incoming unexpected message
...
\mpifunc{MPI_Testsome}.  Note that without the \mpifunc{MPI_Testsome}
call, this requirements of \mpifunc{MPI_Waitsome} won't be met; in
...
exists, use \mpidconst{MPID_Hid_control} to return a remote error indication.
...
change the various routines?  For example, does a \shmem\ device not
...
the group of the window object.  Save the \mpids{MPI_Win}{buffer},
\mpids{MPI_Win}{buffer_size}, and \mpids{MPI_Win}{displacement_unit}. 
...
addresses from all of the processes using \mpifunc{MPI_Allgather},
...
\mpids{MPI_Win}{_flags} with \mpidconst{MPID_WIN_CONST_DISP} and save
the displacement value in \mpids{MPI_Win}{gdisp}.  Otherwise, save the
displacements in an array \mpids{MPI_Win}{displs}.  Save the base
addresses in \mpids{MPI_Win}{bases}.  

Question:  In support of the SPMD, collective allocation model, should
there are also be a \code{gbase} when all processes have the same base
address for the local windows?  If so, use
\mpidconst{MPID_WIN_CONST_BASE} in \mpids{MPI_Win}{_flags}.
...
If the info key \mpiconst{nolocks}\index{MPI_Info!nolocks} is \code{true},
...
fact in the \mpiconst{MPID_WIN_NO_LOCKS} in \mpids{MPI_Win}{_flags}.
...
\mpifunc{MPI_TYPE_GET_ENVELOPE} is called.
