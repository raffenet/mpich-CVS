  ...
  \item[bnr]Process manager interface
    \begin{description}
      ...
      \item[mpd]Implementation of \code{bnr} in terms of \code{mpd}, 
        including the mpd \file{configure}.  Note that mpd is available as
...
% [BRT] Alternative language bindings?  C is out primary binding...
    \begin{description}
...
\item The first line must set the C style to mpich so that there is a common
      ...
      of the predefined styles (such as BSD).
% [BRT] The elisp code for the style should be included in the distribution.
...
\item The first executable statement starts the timer (and in general,
...
\item The example shows the use of MPID routines to lock the communicator
  against other threads.  Note that locks should be used sparinly; any
...
% in a separate block to make it clear what is protected and what is not.
%
...
timer (and any profiling) with \code{MPID_TimerStateEnd}.
...
The program (actually a Perl script) \code{simplemake} reads
...
\file{Makefile.sm} that are not meaningful to the program that processes these
...
\item Check if executing inside a layered routine (i.e., an MPI routine called
  ...
  Section~\ref{sec:err-handling-nested}; the \code{MPIR_Return} macro makes
  this test.
...
example,\index{MPIR_Param_init}\index{MPIR_Param_get_int}%
\index{MPIR_Param_get_string}\index{MPIR_Param_finalize}%
\index{MPIR_Param_bcast}\index{MPIR_Param_register}
\begin{verbatim}
int MPIR_Param_init( int *argc, char **argv[] );
int MPIR_Param_bcast( );
int MPIR_Param_register( const char name[], const char envname[], 
                         const char description[] );
int MPIR_Param_get_int( const char name[], int default_val, int *value );
int MPIR_Param_get_string( const char name[], const char *default val,
                           char **value );
...
\code{MPIR_Param_get_int}. 
The routine \code{MPIR_Param_init} is called by the master process (the one
...
routine \code{MPIR_Param_bcast} is called within \code{MPI_Init} or
...
The routine \code{MPIR_Param_register} allows the MPICH implementation to
...
allows \code{MPIR_Param_finalize} to identify unused command-line arguments
...
The \mpidfunc{MPIR_Param_init} and \mpidfunc{MPIR_Param_free} allows
...
For example, if \mpidfunc{MPIR_Param_get_int} is called but the value
...
An alternative to \code{MPIR_Param_register} is to use a tool, similar to that
...
% \end{verbatim}
\centerline{\psfig{file=intercomm.eps}}
