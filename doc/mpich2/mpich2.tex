...
would like to ensure that MPI programs do not need to load the
...
(that is, these are not global process identifiers).  Two such arrays
are often used, one to map from local PIDs to local ranks
(\code{lpid_to_lrank}) and one to map in the other direction
(\code{lrank_to_lpid}).  
...
\cfgoption{--enable-commid-cache=n} the configure option?  Or, as
above, should it be \cfgoption{--enable-comm:id-cache=n}?
