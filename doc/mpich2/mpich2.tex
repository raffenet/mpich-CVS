...
\begin{description}
\item[mpich/src/include]: This should be the home only for files that are
  common to 
...
location.  See \file{mpich/src/mpi/timer/configure.in} for an example. 
...
\item[mpich/configure.in]: This should have only the tests necessary for the
  code in 
...
\code{AC_CONFIG_SUBDIRS} .  In particular, no tests for features needed by
BNR, the 
timer, or the device should be made here. 
...
yet been implemented, files should not be created for those routines.
...
\code{emacs}, we settled on simply setting the indentation level.
...
\item The declaration of \code{FCNAME}\index{FCNAME} ensures that all macros and
...
\code{int} by the MPI standard but which are just another opaque object.
We
...
\begin{description}
\item[\code{MPI_COMM_NULL}]       \code{0x00000000}
\item[\code{MPI_GROUP_NULL}]      \code{0x04000000}
\item[\code{MPI_TYPE_NULL}]       \code{0x08000000}
\item[\code{MPI_FILE_NULL}]       \code{0x0c000000}
\item[\code{MPI_ERRHANDLER_NULL}] \code{0x10000000}
\item[\code{MPI_OP_NULL}]         \code{0x14000000}
\item[\code{MPI_INFO_NULL}]       \code{0x18000000}
\item[\code{MPI_WIN_NULL}]        \code{0x1c000000}
\item[\code{MPI_KEYVAL_INVALID}]  \code{0x20000000}
\end{description}
...
This section should explain the new/delete, alloc/free, create/destroy set of
names, and mention that the MPI ``free'' routines tend only to decrement a
reference count and free if zero.  To avoid conflicts with the first two pairs
of names, we use create/destroy for routines that create and recover data
structures.
...
standardized includes (e.g., \file{mpiimpl.h}).  
...
\code{AC_CONFIG_SUBDIRS}.  In addition, the changes to the autoconf
...
(\code{MPID_Keyval}).  The keyval does retain the type of object for
...
\mpidconst{MPID_COMM}.  Fills in the fields of the returned
...
\code{MPID_Attr_find} routine (to ensure that an insert or delete of a
...
An alternative approach that is linear is described in the rationale.
...
then call \mpidfunc{MPIR_Datatype_free}.  Invoke the common attribute list
delete code.
...
duplicated and in part because the duplicated code will be close by in the
...
Call \mpidfunc{MPID_Request_probe}.  An implementation of this routine 
...
Call \mpidfunc{MPID_Request_cancel}, which may be implemented as follows:
...
Return \mpidconst{MPID_COMM_PARENT}.
