...
% Define a TeX command that can be used by files to determine whether they
% are being used within this document or in a stand-alone fashion.
\def\indocument{\relax}
...
%describes the MPICH2 test suite, including instructions for extending
%the tests.
...
%      includes handle to pointer transfers.
    ...
    \item[misc]Runtime (Section~\ref{sec:runtime-env}), profiling
      ...
      the implementation of Info is provided in \file{src/util/info}.
...
%      time-stamp routines.
      ...
      README.}Some documentation on \file{rlog} is in \file{doc/logging}.
  ...
  \item[mpid]Implementation of the ADI3.  There are many
    ...
    \begin{description}
      ...
      relying (for now) on the \code{poll} system call instead of the
      \code{select} call.
    \end{description}
    \item[ch3]The ADI-3 version of the channel interface.  A number of
      ...
      These are contained in the directory \file{channels}, with a TCP
      implementation, 
      \begin{description}
...
% is likely to exist only in or for MPICH.
  ...
  specific to compilers for the respective languages.
    ...
    directory names.  
    \item[f77]Fortran 77 binding (includes \file{configure}).
    \item[f90]Fortran 90 binding (includes \file{configure}).
...
\item[doc]Documentation
  \begin{description}
    ...
    process manager.
    \item[logging]Describes the \file{rlog} logging mechanism.
  \end{description}
    ...
    \item[mpi]Test of MPI.  Must work with \emph{any} MPI implementation.
      \begin{description}
        \item[\emph{xxx}]The directory structure matches that for the
          \file{src/mpi}, at least for the major sections.  
...
directory will also contain the file \file{libpmpich.a}.
...
%The major issues include support
...
\item[src/include] is be the home only for files that are
  common to 
the implementation of the routines in \file{src/mpi}, and is not be
the 
...
%% necessary include files into \file{src/include} from their natural home
%% location.  
...
replacing, values).
See \file{src/mpi/timer/configure.in} for an
example. 
...
vpath\index{vpath} (virtual path feature of many \code{make} programs) builds.

\item[Global variables] should be grouped together by module.  For the routines
in \file{src/mpi} (but not counting \file{src/mpi/timer} because that is
a separate 
...
\item The structured comment block uses predefined text for the possible error
  ...
  names is provided in the file \file{maint/docnotes}.
...
\item There are some predefined error tests with the form
  ...
  \code{MPIR_ERRTEST_INITIALIZED_ORDIE}.  These predefined tests are defined
  in the file \file{src/include/mpierrs.h}.
...
/* Any internal routines can go here.  Use PMPI_LOCAL for routines
   that are local to this routine only. */
...
/*@
   ...
   Input Parameters:
+  first - text for first parm
.  middle - ditto for second
-  last - text for the last parm.  

   Output Parameter:
.  only - a single parameter

   Notes:
   Any brief discussion of the routine
...
.N ... others
@*/
int MPI_Foo( MPI_Comm comm, int middle, int *last, int *only ) 
{
...
#   ifdef HAVE_ERROR_CHECKING
    {
        MPID_BEGIN_ERROR_CHECKS;
        {
            ...
            /* Use predefined macros for tests where possible */
            if (a < 0) {
...
to the MPICH library (normally \file{libmpich.a}; the name may be
changed by setting the environment variable \code{MPILIBNAME}).
...
\code{targetname-local}.  For example, to add commands to the clean
target, use
\begin{verbatim}
clean-local:
        rm -f myoptprog
...
The \file{mpif.h} (actually \file{mpif.h.in}) that is used with Fortran is
created from 
\file{mpi.h.in} so that the various integer values (e.g., error classes, 
...
\file{configure.in}, using the scripts \code{buildiface} in the
\file{src/binding/f77} directory.  The point is to automate this and
ensure that the \file{mpif.h.in} file
is created from the \file{mpi.h} automatically, at least with
...
\file{src/mpid/mydevice}) will never be combined with other
...
\file{src/include/mpierrs.h}.  These handle the various error levels
...
return; by default, it will abort.  A return may be prefered for some
fault-tolerant applications.  This routine is presently called from the
\code{MPIR_ERRTEST_INITIALIZED_ORDIE} macro if MPI has not been properly
initialized.  In turn, this macro is called at the beginning of each MPI
routine.
...
To access this data, the routine \code{MPIR_GetPerThread} is used.  This
has the following definition:
\begin{verbatim}
#if (MPICH_THREAD_LEVEL < MPICH_THREAD_MULTIPLE)
...
\fixme{there are a number of undocumented varients of these that need
  to be explained or removed, such as {func}\_{enter}\_{front}}
...
\file{include/mpistates.h}.  However, alternate definitions can be
used by redefining
...
controlled:
\begin{description}
\item[at compile time]with the \cfgoption{--enable-timing} configure
  flag.
%% The following good idea was never implemented.
...
%% The ``at run time'' argument was never implemented and cannot be
%%  without the ``param'' support.
...
majority of the MPICH2 team has decided to rely on \code{getenv}
instead.

% The old text is in parm.tex 
...
%%
%% The majority view is that these are unimportant.  I've removed them
%% so that readers will not be confused about what we claim to do
...
%% These are recommendations that have
%% occasionally been followed.
...
.N Deprecated
\end{verbatim}
%% The manual pages for the deprecated routines should make clear
...
% It would be a good idea not to set CFLAGS.  But this seems too hard
% for GNU autoconf.
...
%% I've removed this section because the majority of the MPICH2 team 
%% disagrees with this approach.  The majority opinion is to perform
%% ``reactive'' coding, fixing problems when they are obsevered,
%% rather than to benefit from long experience and try to avoid them
%% in advance.
...
%% The majority of the team disagrees with the following.
...
% The following is unimplemented and unlikely to be
...
%% The majority of the MPICH2 team did not agree with these goals and 
%% procedures for implementing testing.
%\section{Testing}
%\label{sec:testing}
...
\file{src/misc2}.  The other is to implement only the defined keys that
...
\mpifunc{MPI_Type_get_name}), however, is set only if 
\mpifunc{MPI_Type_get_name} or \mpifunc{MPI_Type_set_name} is used.
...
along with a variable containint the smallest context ids of the 
communicators that
...
thread-shared memory (in this case, process-local variables).\index{thread
safety!context allocation}
In this code, note that \code{mask_in_use} is initialized to zero.

\begin{algorithm}
static volatile int mask_in_use = 0;
static volatile int lowestContextId = MAXINT;
int context_id = 0;
while (context_id == 0) {
    lock 
    if (mask_in_use || mycontextid > lowestContextId) 
        local_mask = 0
    else 
        local_mask = mask
        mask_in_use = 1
    if (mycontextid < lowestContextId) 
        lowestContextId = mycontextid
    unlock
...
Another refinement of this algorithm would use a queue of pending
threads, ordered by increasing contextid value (of the input
communicator).  Threads that are high in this queue could wait on a
condition variable or other thread-synchronization mechanism that is
activated whenever there is a change in the thread with the lowest
context id, either because a thread has found a context id value and
is removed from the queue or because a new thread with a lower context
id value than any of the threads currently attempting to acquire a
context id enters the function.  Since most applications will are
unlikely to have large numbers of threads attempting to create context
ids at the same time, the simple algorithm is probably sufficient.
...
If a cache is supported, one of the issues is ensuring that in a
...
\file{src/util/bsendutil2.c} in MPICH-1.  Note that while the buffer is
...
% the case where the data is delivered in several parts.
...
Call \mpidfunc{MPID_Alloc_mem}.  We also need a routine that
...
allocated using \mpifunc{MPI_Alloc_mem}.  The info argument could be
used to express the intended use of the space, alllowing
\mpifunc{MPI_Alloc_mem} to select an appropriate memory pool.
