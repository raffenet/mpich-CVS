...
example,\index{MPIi_Param_init}\index{MPIi_Param_get_int}\index{MPIi_Param_get_string}\index{MPIi_Param_free}
\begin{verbatim}
...
\mpidfunc{MPIi_Param_get_int} is called but the value is \code{"big"}?
...
Error classes include \mpiconst{MPI_ERR_TYPE} and
\mpiconst{MPI_ERR_ARG}. 
...
stored in C style (null terminated) in all MPI datastructures.
...
that are not dependent on the rank in the communicator.
\end{adi3}
...
Data this is sent with \mpiconst{MPI_PACKED} as the datatype may either be
...
\item Source process uses \mpifunc{MPI_Pack} and sends using type
  ...
  \item The receive type is \mpiconst{MPI_PACKED}.  The data must be copied
    ...
    format \emph{and source}, stored in the header.
...
Error classes include \mpiconst{MPI_ERR_GROUP}, \mpiconst{MPI_ERR_RANK}, and
\mpiconst{MPI_ERR_ARG}. 
...
suggests the use of an array of ranks to the ADI-3 local process ids.  Local
...
\code{size} of \mpiconst{MPI_COMM_WORLD} $-1$.  Processes that are added to a
...
\item For \code{group2}, if necessary, create a new array containing the pairs
  ...
  (\mpids{lpid_to_lrank}{lrank}), sorted by \code{local process 
    id}. Call this array \mpids{MPI_Group}{lpid_to_lrank}; the elements are
  structures of type \mpidconst{MPID_Group_pmap_t}.  
...
This should also be a runtime parameter.
...
not in \code{group1} (check the \mpids{MPI_Group}{lpid_to_rank} array for
...
context ids, at least in the single-threaded case.
...
in the same new communicator.  Create the subgroups,
ordered according to the key (using an array containing (key,ownerrank), sort
by key, then extract the \code{lpid} from \mpids{MPI_Group}{lrank_to_lpid}.
Use \mpifunc{MPI_Comm_create} to build the new communicator.
...
the C++ binding makes a \code{Comm} an abstract base class, so you can't dup a
