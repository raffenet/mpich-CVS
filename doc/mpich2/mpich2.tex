...
%
% Algorithm
\newenvironment{algorithm}{\begin{obeylines}\obeyspaces}{\end{obeylines}}
...
memory.  However, most user memory (i.e., user-declared
...
\begin{adi3}
\mpidfunc{MPID_Segement}, \mpidfunc{MPID_Stream_irecv}, and
\mpidfunc{MPID_Streadm_isend}. 
\end{adi3}
...
\mpidconst{MPID_Hid_datatype_desc}?  Should the lock/unlock operations
...
\mpidfunc{MPIi_Op_setup} and \mpidfunc{MPIi_Op_finalize}?
...
could have \mpidfunc{MPIi_Topo_init}.
...
barrier tree).  Or it just use \mpifunc{MPI_BARRIER}.  
...
private communicator that can be used as necessary; this also stores
...
\mpifunc{MPI_Win_lock} and \mpifunc{MPI_Win_unlock} calls.

\subsubsection{\mpifunc{MPI_WIN_FREE}}
Call \mpifunc{MPI_Barrier} on the internal \mpids{MPI_Win}{communicator}.
Check for errors, such as unreleased locks or incomplete
post/start/complete/wait synchronization.
...
Sets the \mpids{MPI_Win}{name} field.  Returns error if supplied name
is too long. 
...
queueing up a long list of operations.
