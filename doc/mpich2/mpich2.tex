...
operations (see the paper on improving TCP performance \cite{??}).  If so, we
...
other person who is the ``immediate reviewer''.  The writer is responsible for
...
Once a section is written, it should be read and by everyone and we should
...
step is completed, the routines in the section can be written.

Many of these sections can be implemented independently.  
\begin{enumerate}
\item Infrastructure
  \begin{enumerate}
    \item Directory structure
    \item MPI routine source code template
    \item mpich2 bug list (first entry: \file{config/confdb} is wrong; it
      appears to be a snapshot (complete with bugs) of the correct one).
  \end{enumerate}
\item MPI Major Sections
  ...
  \item Collective.  Most of this can be left until later.  However, it
    would be good to include details for \mpifunc{MPI_Bcast} and
    \mpifunc{MPI_Scatter} that show how the routines use the lower-level
    point-to-point routines (e.g., what is called ``stream'' in this
    document).  Also:
    \begin{enumerate}
  ...
  \begin{enumerate}
    \item Scenarios
  \end{enumerate}
  \item Starting and Ending MPI.
  \item Dynamic processes.
  \item User-defined requests.
  \item Error handlers.
  \item Handle Transfers.
  \item Timers.
...
\item Source code, portability, and framework
  ...
  \item Testing.  Needs new test harness; intelligent sampling of the possible
    combinations; archiving of results (including performance tests).
  \end{enumerate}
\item Minor Items
  \begin{enumerate}
