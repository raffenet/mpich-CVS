...
\end{description}
The macro \code{HANDLE_KIND(id)} returns this value as an integer in
the range $0$--$7$.
...
additional elements to be allocated dynamically as required.
...
used by handles with type \code{CONSTRUCT_DIRECT}.  The dynamically
allocated array 
(actually an array of pointers to arrays) is used for handles with
type \code{CONSTRUCT_INDIRECT}.  Elements are allocated from these
...
first info element (using the otherwise ununsed \code{key} and
