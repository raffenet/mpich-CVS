...
#   ifdef HAVE_ERROR_CHECKING
    {
        MPID_BEGIN_ERROR_CHECKS;
        {
            ...
            if (mpi_errno) {
                ...
                return MPIR_Err_return_comm( comm_ptr, mpi_errno, FCNAME );
            }
        }
        MPID_END_ERROR_CHECKS;
    }
...
of \file{.}).  It also adds \file{wrapack.o} to the \file{libmpich2} library.
...
utility routine.  See \file{src/util/info/infoutil.c} for an example.
...
#define MPID_GetPerThread(p) {\
     ...
     if (!p) { p = MPIU_Calloc( 1, sizeof(MPID_PerThread_t ) );\
...
The \file{tr2} (TRacing memory package) provides both tests for memory
