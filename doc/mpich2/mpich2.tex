...
\item[src]Source files for the MPI implementation
  ...
  \item[mpi]The implementation of the MPI routines.  Routines in this
    ...
    \item[datatype]Datatypes (Section~\ref{sec:datatypes}), including the
      ...
      \code{MPI_Unpack_external} functions.
...
%      includes handle to pointer transfers.
    \item[timer]Timers (Section~\ref{sec:timer}).  This is separate from the
      ...
      directory will have its own \code{configure}.
...
% (installed) to the public, they are really part of the source.
...
% to order the building of modules based on dependencies.
  \item[util]Various utilities.  These have MPIU prefixes.
    ...
    \item[mem]Memory allocation and management.  These include the tracing
      malloc routines (\file{tr2} in the MPICH implementation).
...
\item The first line must set the C style to mpich so that there is a common
      ...
      \code{eval} line must be used.  An \code{mpich} style was chosen so that 
      we can specify the behavior; another approach is to adopt one
      of the predefined styles (such as BSD).  If we use our own
      style, the elisp code for it will be included in \file{maint}.
...
\item The two lines the undefine \code{FUNCNAME} and then define
...
\item The structured comment block uses predefined text for the possible error
  ...
  errors \code{.N Errors}.
\item The declaration of \code{FCNAME} ensures that all macros and
...
\code{MPID_Comm_incr} etc.) instead of lock/unlock.  
...
manually update each \file{Makefile.in}.
...
\file{makeconfigure}\index{makeconfigure} script in the top level directory.

Note: \code{simplemake} needs to move into the \file{mpich2/maint}
...
possible, then it should be accessed through \code{MPIR_} or \code{MPID_}
routines.  
...
Fortran compilers and the \code{MPI_xxx_c2f} functions.
\end{itemize}
...
\mpiconst{MPI_Info}, \mpiconst{MPI_Op}, and \mpiconst{MPI_Win}.  We
...
% \end{description}
% In addition, the macro \code{CONSTRUCT_MASK} may be used to return
% just these bits and \code{CONSTRUST_TYPE(id)} returns these values as the
% integers $0$--$3$.
...
\begin{description}
\item[\code{MPI_Comm}]     (000) \mpidconst{MPID_COMM}
\item[\code{MPI_Group}]    (001) \mpidconst{MPID_GROUP}
\item[\code{MPI_Datatype}] (010) \mpidconst{MPID_DATATYPE}
\item[\code{MPI_File}]     (011) \mpidconst{MPID_FILE}
\item[\code{MPI_Errhandler}](100) \mpidconst{MPID_ERRHANDLER}
\item[\code{MPI_Op}]       (101) \mpidconst{MPID_OP}
\item[\code{MPI_Info}]     (110) \mpidconst{MPID_INFO}
\item[\code{MPI_Win}]      (111) \mpidconst{MPID_WIN}
\end{description}
The macro \code{HANDLE_MPI_TYPE(id)} returns this value as an integer in
the range $0$--$7$.  The names belong to the enum type
...
\code{MPID_Comm_get_ptr} converts an \code{MPI_Comm} handle to a pointer to an
\code{MPID_Comm *} pointer.  The macro \code{MPID_<type>_valid_ptr} confirms
...
object and the \code{ref_count} is the reference count.
...
lists.  However, this can encur a significant overhead, particularly with
...
while (ptr = avail) {
    ...
    nxt = ptr->next; 
    flag = 0
    asm( %eax = avail; lock ; cmpxchg avail,nxt ; sete flag );
    if (flag) break;
}
\end{verbatim}
...
actually an opcode prefix that causes the \code{cmpxchg} instruction to happen
atomically (this is the default on some but not all Pentiums).
Note that this code does not require any thread locks.
...
\end{verbatim}
As above, this \code{asm} code is not correct on simply sketches the
operations that are necessary.
In both of these cases, the \mpids{MPIR_Process}{allocation_lock} is still
...
\begin{verbatim}
   ...
   return ptr;
\end{verbatim}
For time-sensitive handles such as \code{MPID_Request}s, it is possible to
inline the basic code (atomically implementing the \code{ptr=avail;
  avail=avail->next}) and call a routine in the case that \code{avail} was
null.  
...
\item Are output parameter pointers valid?  The macro
  \code{MPID_Pointer_is_invalid(void *p)} returns one for invalid pointers.
  ...
  \code{SIGSEGV} handler set.  An enhancement of this is to return
...
testing that \code{buf + datatype->real_lower_bound} is not null.  If we use
...
\item Otherwise, for everything except MPI-IO, use the error handler attached
  ...
  \code{MPI_COMM_WORLD}.'' ).  Ensure that the manual pages on error handlers
...
\begin{verbatim}
    return MPIR_Err_return_comm( comm_ptr, error_code, FCNAME );
\end{verbatim}
...
between two timestamps into a number of seconds (a \code{double}).  These
...
% enable-timing=all,class=pt2pt,class=coll.
...
\code{select} should be covered.  We may want to have a class for idle time
and separate entries for each place where the code waits.  For example,
\code{MPID_STATE_IDLE_MPI_WAIT}.  
...
function name (e.g., \code{MPID_STATE_SYS_GETHOSTBYNAME}).
...
value (e.g., the \code{val} in the example).
\end{description}
...
constant values that are set in the file \file{mpistat.h}.  We may provide a
...
\mpifunc{MPI_Finalize}. (See Section~\ref{sec:finalize}.)  This end-of-job
...
The \file{util/mem/trmem} (TRacing MEMory package) provides both tests for
memory 
...
\cfgoption{--enable-g=trmem} or \code{--enable-g=all}.
...
The routine \code{MPIU_Param_register} allows the MPICH implementation to
...
allows \code{MPIU_Param_finalize} to identify unused command-line arguments
...
For example, if \mpidfunc{MPIU_Param_get_int} is called but the value
...
Currently these are in \file{include/mpiimpl.h} and
\file{src/util/thread/gthread.c}.  
...
\code{MPI_Init} and \code{MPI_Init_thread} call a series of
...
used only in the MPICH (and not MPID) code, and \code{MPID_} for all
...
autoconf macro \code{AC_C_INLINE} to test for this feature.
...
A common indentation style, \code{mpich}, is specified for all files.
...
This is interpreted by Emacs and allows us to control the 
indentation style for MPICH 
...
\item[File header.]
There is a standard file header, \file{topfile.c}, containing the copyright and
standardized includes (e.g., \file{mpiimpl.h}).  This header
...
for setting this variable; it should be of type \code{static const char[]}.
...
\code{CONSTRUCT_BUILTIN} type and encode the length of the datatype in bytes
...
\item Setup the two dataloops.  For datatypes that require copies of input
  ...
  \mpiconst{MPI_COMBINER_HVECTOR} is stored in the combiner fields in the
  datatype.  
  ...
  \begin{enumerate}
    ...
    \item Are the elements aligned on a multiple of the element size?
      ...
      element-size field in \mpids{MPID_Dataloop}{kind} is set to 1, 2, or 3
...
\mpidfunc{MPIR_type_signature}.  The configure option
...
gives a byte address (assumes that a \code{char} is a byte). 
...
\mpidfunc{MPIR_Datarep_finalize}? 
...
\mpids{MPIR_Bsend_elm}{communicator},
\mpids{MPIR_Bsend_elm}{rank},
\mpids{MPIR_Bsend_elm}{datatype},
...
managers and related resources.  The external process manager may be
very sophisticated and offer many useful functions or it may be very
simple and capable of few operations beyond starting a process.  The
BNR interface provides an interface that allows us to exploit the
...
identified by (group, rank).  Thus, these are different from MPI groups.
...
with a job and keymap handle.  Its purpose is to provide certain
...
to other processes in the same job, with information on who they may
...
environments where it is easy for multiply groups to share the same keymap.)
...
insert (put) or extract (get) information from the keymap.  If a member of
...
a new keymap using BNR and populate the keymap with the key-value pairs it
...
same keymap domain, and that extracting and communicating all of the data
within a keymap would be unnecessary and costly.  We realized that if we
associated a keymap to some notion of a domain rather than a particular
...
Question: Why are keymaps (old databases) managed by string name
instead of a handle?  If a serialized name is needed for external
identification, that could be given by a separate function.  - WDG.

\begin{verbatim}
  int BNR_Key_Get_my_name(char *dbname)        - get name of keymap
  int BNR_Key_Get_name_length_max( )           - needed since might be sent
  int BNR_Key_Create(char *dbname [OUT})       - make a new one, get name 
  int BNR_Key_Destroy(char *dbname [IN])       - finish with one 
  int BNR_Put(char *dbname,
              const char key[BNR_MAX_KEY_LEN],
  ...
  int BNR_Key_iter_first(char *dbname, char *key, char *val)  - loop through the
  int BNR_Key_iter_next(char *dbname, char *key, char *val)      pairs in the db
\end{verbatim}
  
On a \code{BNR_Put}, if a pair with the same key is already in the keymap,
...
groups are separate from keymaps.  Since \code{BNR_Put}s and \code{BNR_Get}s
...
\item[gcc -Wall etc.]Missing prototypes, return values, etc.  This can be set
  in \code{configure} with the \code{PAC_ARG_STRICT} macro.
...
messages).  Question: what is the \code{--enable} value for this?
...
optimizes for small numbers of objects (the \code{CONSTRUCT_DIRECT} type) but
...
\item Removes bogus targets for updating the \code{configure} and
  ...
  removing the \code{stamp-h} target and dependencies.
\item Cleans up empty targets and dependencies.  While not essential, the
  \file{Makefile.in}s generated by \code{automake} are very messy; user's are
