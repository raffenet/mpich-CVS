...
routines.  Section~\ref{sec:mpi-operations} describes the implementation of
...
\item[src]Source files for the MPI implementation
  ...
  \item[mpi]The implementation of the MPI routines
    ...
    \item[comm]Communicators (Section~\ref{sec:communicators})
      \textbf{Question: intercomm separate?}
    \item[pt2pt]Point-to-point (Section~\ref{sec:pt-2-pt})
    \item[coll]Collective communication and computation
      (Section~\ref{sec:collective-comm}) 
      \textbf{Question: split into intercomm and intracomm?}
    ...
    \item[misc]Runtime (Section~\ref{sec:runtime-env}) and profiling
  ...
  \item[util]Various utilities
    ...
    \end{description}
  \item[mpid]Implementation of the ADI3
  \item[bnr]Process manager interface
    \begin{description}
      \item[include]BNR header files
      \item[mpd]Implementation of \code{bnr} in terms of \code{mpd}
        \begin{description}
          \item[src]Source files for mpd
          \item[include]Include files for mpd
        \end{description}
    \end{description}
  \item[binding]Language bindings
    \begin{description}
    \item[c++]C++ binding (includes \file{configure})
      \begin{description}
      \item[src]
      \item[include]
      \end{description}
    \item[fortran]Fortran 77 binding (includes \file{configure})
      \begin{description}
      \item[src]
      %\item[include]
      \end{description}
...
\textbf{Should most include files go here?  What about replacable modules,
  ...
  On possibility is to use links to the files that would be set up as part of
  the build process, rather than including a long list of directory paths.
  For Windows, copies of the files could be used.
...
\item[maint]
  ...
  distributed.
  \begin{description}
  \item[config]\code{autoconf} macro files
  \end{description}

\item[test]Testing tools and programs
  \begin{description}
    \item[util]Contains utilities for running the tests
...
\item The first line must set the C style to mpich so that there is a common
      ...
      we can specify the behavior; another (riskier) approach is to adopt one
      of the predefined styles (riskier because it might change).
...
\item The first C statement must be the include of \file{mpiimpl.h}.  This
  ensures that the configuration switches (in \file{mpichconf.h}) as well as
  all include files defining the various MPICH internals.
...
\item The first executable statement starts the timer for the routine.  This
  is a macro that will expand into the appropriate code (including no code for
  the fastest production version).
\item The actual error checks are guarded by both a compile-time test and a
  ...
  issued (this allows us to consider adding code to catch \emph{all} errors).
\item Error codes are generated with \code{MPIR_Err_create_code}.  These use
  predefined name strings (see error reporting in the ADI manual).
\item The return always gives \code{MPI_SUCCESS} explicitly.  Any return that
  might return an error code should use \code{MPIR_Return} instead.  
\end{enumerate}
...
int MPI_Foo( MPI_Comm comm, int a ) 
{
    ...
    MPID_BEGIN_ERROR_CHECKS;
        if (MPIR_Process.initialized != MPIR_WITHIN_MPI) {
        ...
        if (mpi_errno) {
            MPID_TimerStateEnd(MPID_STATE_MPI_FOO);
...
with a separate name.  The suffix \code{pf} is used instead of \code{po}
because \code{po} is used by the GNU \code{gettext} facility for
internationalization.  These use a different suffix (\code{pf}) from that used
...
\item[MPIR]Routines used only within the MPI implementation (outside of the
  ADI).
\item[MPID]Routines defined either in the ADI or used within the ADI
\end{description}
...
\begin{verbatim}
#define BEGIN_ERROR_CHECK if (MPIR_Process.do_error_checks) {
...
The test \code{MPIR_Check_user_buffer( buf, count, datatype )} checks that the
...
handler.  This handler may either abort or return; by default, it will abort.
...
#define MPIR_GetPerThread(p) p = &MPIR_Thread
#else /* Assumes pthreads for simplicity */
#define MPIR_GetPerThread(p) {\
...
counter.  These are described in detail in the ADI3 document.
...
\subsubsection{Instrumenting the MPI code.}
...
timestamps, along with the number of calls.
...
Each state belongs to a class; there are 32 classes (so that we can use
bitwise tests on a 32 bit \code{unsigned int}).  Instrumentation can be
controlled:
\begin{description}
\item[at compile time]with the \code{--enable-timing} configure flag.
...
Where it is necessary to allocate memory, the function \mpidfunc{MPID_Malloc}
(and corresponding \code{MPID_Calloc}, etc.) must be used instead of
...
flagged as an error by the code style checkers.  Note that \code{MPID_Malloc}
...
to using the \code{MPID_Malloc} routines.
...
routine \mpidfunc{MPID_Trdump} if the runtime parameter
...
local instance (for thread-safety) of \code{MPID_Mem_stack} 
...
example,\index{MPIi_Param_init}\index{MPIi_Param_get_int}%
\index{MPIi_Param_get_string}\index{MPIi_Param_finalize}%
\index{MPIi_Param_bcast}\index{MPIi_Param_register}
\begin{verbatim}
int MPIi_Param_init( int *argc, char **argv[] );
int MPIi_Param_bcast( );
int MPIi_Param_register( const char name[], const char envname[], 
...
\code{MPIi_Param_get_int}. 
The routine \code{MPIi_Param_init} is called by the master process (the one
...
routine \code{MPIi_Param_bcast} is called within \code{MPI_Init} or
...
The routine \code{MPIi_Param_register} allows the MPICH implementation to
...
allows \code{MPIi_Param_finalize} to identify unused command-line arguments
...
An alternative is to use a tool, similar to that for error reporting, that
finds all uses of the parameter routines.

Question: How do we find out that no value was specified?  This is different
from having a default value.

Question: the file read operation is a potential problem.  Do we want to have
a separate pre-init and post-init (that can use \code{MPI_Bcast}) to get data?
...
/* -*- Mode: C; eval: (c-set-style "mpich"); -*- */
\end{verbatim}
This is interrepted by Emacs and allows us to set a style for MPICH
...
an MPICH distribution and will 
...
list:
\begin{description}
\item[\texttt{CROSS_BYTE_ORDERING}]\code{WORDS_BIGENDIAN} or empty (just like
  \code{AC_C_BIGENDIAN})
...
\subsection{Memory Allocation}
The use of \code{MPID_Malloc} etc. makes it easier to use portable versions of
...
int MPIi_Param_bcast( void ) { return 0; }
int MPIi_Param_get_int( const char name[], int default_val, int *value ) 
