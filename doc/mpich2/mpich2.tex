...
\item[src]Source files for the MPI implementation
  ...
  \item[mpi]The implementation of the MPI routines
    ...
    \item[grequest]User-defined requests (Section~\ref{sec:grequest})
      \textbf{Question: Part of point-2-point?}
    \item[errhan]Error handlers (Section~\ref{sec:errhand})
    \item[handle]Handle transfers (Section~\ref{sec:handle-transfer})
    \item[timer]Timers (Section~\ref{sec:timer})
...
\item Are output parameter pointers valid?  The macro
  ...
  \code{(long(p)<=0)} or \code{9!(p) || (unsigned long)(p) > STACKLIMIT)}, or
...
\index{MPID_Nest_incr}\index{MPID_Nest_decr}\index{MPID_Nest_value}
\begin{verbatim}
void MPID_Nest_incr()
void MPID_Nest_decr()
int MPID_Nest_value()
\end{verbatim}
The macro \code{MPIR_Return} can be used:
\begin{verbatim}
    MPIR_Return( errhandler, error_code )
\end{verbatim}
    ...
    { char *tmp = getenv{name); 
      if (tmp) { 
          ...
          *value = strtol(name,&endp,10); 
          /* if name not an integer, return 1 */
          return (*endp != '\0'); }
      *value = default_val; 
      return 0; }
...
For example, the pre-init routine code read just a few environment variables
...
#define MPIR_GetPerThread(p) {\
     ...
     if (!p) { p = MPID_Calloc( sizeof(MPID_PerThread_t ) );\
...
We will specify an indentation style.  If so, we need to
...
For example, \code{/* -*- Mode: C; c-set-style: mpich -*- */}.
...
\item[Function Name.]
The function name is available as \code{FCNAME}.  A configure test will
...
counter.  The routine \mpidfunc{MPID_TimerConvert} converts the difference
between two timestamps into a number of seconds (a \code{double}).  These
provide the basic support for timing on a process.  

\textbf{Question: compare this to the ADI timers}

\begin{verbatim}
typedef ... MPID_Timestamp_t;
MPID_TimerStamp( MPID_Timestamp_t timestamp)
MPID_TimerConvert( MPID_Timestamp_t timestamp1, 
                   MPID_Timestamp_t timestamp2, 
                   double *seconds)
MPID_TimerAcc( MPID_Timestamp_t t1, MPID_Timestamp_t t2, 
               MPID_Timestamp_t t3 )
MPID_TimerInit( )
MPID_TimerFinalize( )
\end{verbatim}
\code{MPID_TimerAcc} is an accumulate function; if the time stamps are numeric
types (e.g., \code{long} or \code{double}), then \code{MPID_TimerAcc} is
simply
\begin{verbatim}
    t3  += (t2 - t1);
\end{verbatim}
...
Question: what about \file{mpio.h}?
...
something like the \code{DLAST} feature of MPICH (e.g., hold the last 100 messages).
...
Or should we just write them out?
