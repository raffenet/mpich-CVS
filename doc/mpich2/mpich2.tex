...
\item The actual error checks are guarded by both a compile-time test and a
  ...
  The expansion of this macro is controlled by the
\cfgoption{--enable-error-checking} to \code{configure}.
...
\item[Keyval] There are a number of predefined keyvals, but these are
...
Figure~\ref{fig:handle-direct} shows
...
There are two basic types of profiling: state recording, ala SLOG
\cite{slog}, and 
...
\item[at compile time]with the \cfgoption{--enable-timing} configure flag.
 ...
 enable-timing=all,class=pt2pt,class=coll.  These are implemented by
...
names for all system routines start with \code{MPID_STATE_SYS_}.  
...
than a lock ofr each item updated.
...
values to be handled with a single lock.
...
We will provide an automated tool to compile a listing of such parameters and
...
Attributes are implemented as a simple linear list on each
communicator.  The major issue with the implementation of attributes
...
names only if \texttt{MPI\_Type\_get\_name} is used by the user's program.}.

\subsubsection{Creating a New Datatype}
Here are the steps to create and return a new datatype.
\begin{enumerate}
\item Allocate a new object using \code{MPIU_Handle_new_obj}.
...
\mpidfunc{MPIR_Type_signature} to compute 	
the representative type signature.
...
Question: should this call an MPID function for systems that want to
manage the count field differently, that is, not use it to store the number of
bytes in the message (e.g., Globus)?  
...
function that understands the datatype?  I prefer a function pointer
...
initialize these names at \code{MPI_Init}.
...
connection array.  Groups are provide primarily to support the MPI
routines that require them.  

Groups are not even allocated for communicators unless they are
required \mpifunc{MPI_Comm_group}.  In other words, we create the
