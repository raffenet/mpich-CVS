...
\file{mpi.h}; the others are defined in the file \file{mpierrcodes.h} which is
...
\item[\mpiconst{MPI_ERR_BUFFER} \emsg{buffer}]Invalid buffer pointer
    \begin{description}
    \item[\mpidconst{MPIi_ERR_BUFFER_NULL} \emsg{bufnull}]Null buffer pointer
    \item[\mpidconst{MPIi_ERR_BUFFER_NOSPACE}
    \emsg{bufbsend}]Insufficient space in Bsend 
      buffer (optional args: requested and avaliable length (int))
    \item[\mpidconst{MPIi_ERR_BUFFER_ALIAS} \emsg{bufalias}]Buffers
    must not be aliased 
      (optional args: names of two arguments (string))
    \item[\mpidconst{MPIi_ERR_BUFFER_SIZE} \emsg{bufsize}]Invalid
    buffer size (optional arg: 
      size (int))
    \item[\mpidconst{MPIi_ERR_BUFFER_BSEND_EXISTS}
    \emsg{bufexists}]Buffer already attached with 
      \mpifunc{MPI_BUFFER_ATTACH}. 
    \item[\mpidconst{MPIi_ERR_BUFFER_BSEND_SMALL}
...
\item[\mpiconst{MPI_ERR_COMM} \emsg{comm}]Invalid communicator
    \begin{description}
    \item[\mpidconst{MPIi_ERR_COMM_NULL} \emsg{commnull}]Null communicator
    \item[\mpidconst{MPIi_ERR_COMM_INTER}
    \emsg{intercomm}]Intercommunicator is not allowed 
    \item[\mpidconst{MPIi_ERR_COMM_INTRA}
    \emsg{intracomm}]Intracommunicator is not allowed 
    \item[\mpidconst{MPIi_ERR_COMM_NAME} \emsg{commname}]Cannot set
    name in communicator 
    \item[\mpidconst{MPIi_ERR_COMM_PEER} \emsg{commpeer}]Peer
    communicator is not valid 
    \item[\mpidconst{MPIi_ERR_COMM_LOCAL_NULL}
...
\item[\mpiconst{MPI_ERR_RANK} \emsg{rank}]Invalid rank (optional
    argument: value (int)) 
    \begin{description}
    \item[\mpidconst{MPIi_ERR_RANK_ARRAY} \emsg{rankarray}]Invalid
    rank in rank array (optional 
      arguments: index, value, size-1 (int))
    \item[\mpidconst{MPIi_ERR_RANK_DUP} \emsg{rankdup}]Duplicate ranks
    in rank array 
      (optional arguments: index, value, other index (int))
    \item[\mpidconst{MPIi_ERR_RANK_LOCAL} \emsg{ranklocal}]Error
    specifying local_leader 
      (optional arguments: value, size-1 (int))
    \item[\mpidconst{MPIi_ERR_RANK_REMOTE} \emsg{rankremote}]Error
...
\item[\mpiconst{MPI_ERR_ARG} \emsg{arg}]Invalid argument (optional
    arg: name (string)) 
    \begin{description}
    \item[\mpidconst{MPIi_ERR_ARG_ERRCODE} \emsg{argerrcode}]Invalid
    error code (optional arg: 
      value (int))
    \item[\mpidconst{MPIi_ERR_ARG_NULL} \emsg{argnull}]Invalid null
    parameter (optional arg: 
      name of argument (string))
    \item[\mpidconst{MPIi_ERR_ARG_F77_ADDRESS}
    ...
    \item[\mpidconst{MPIi_ERR_ARG_ERRHANDLER}
    \emsg{errhandler}]Invalid errhandler 
    \item[\mpidconst{MPIi_ERR_ARG_ERRHANDLER_NULL}
    \emsg{errhandlernull}]Null errhandler 
    \item[\mpidconst{MPIi_ERR_ARG_ERRHANDLER_FREE_PERM}
...
\item[\mpiconst{MPI_ERR_REQUEST} \emsg{request}]Invalid \mpiconst{MPI_Request}
    \begin{description}
    \item[\mpidconst{MPIi_ERR_REQUEST_NULL} \emsg{requestnull}]Null
    \mpiconst{MPI_Request} 
    \item[\mpidconst{MPIi_ERR_REQUEST_NOT_PERSISTENT}
...
\mpifunc{MPI_File_open} 
  (optional argument: amode (int))
    \begin{description}
    \item[\mpidconst{MPIi_ERR_AMODE_ONLY_ONE}
...
\item[\mpiconst{MPI_ERR_FILE} \emsg{file}]Invalid \mpiconst{MPI_File}
    \begin{description}
    \item[\mpidconst{MPIi_ERR_FILE_NULL} \emsg{filenull}]Null
    \mpiconst{MPI_File} 
    \end{description}
\item[\mpiconst{MPI_ERR_INFO} \emsg{info}]Invalid \mpiconst{MPI_Info}
    \begin{description}
    \item[\mpidconst{MPIi_ERR_INFO_NULL} \emsg{infonull}]Null
    \mpiconst{MPI_Info} 
    \end{description}
\item[\mpiconst{MPI_ERR_INFO_KEY} \emsg{infokey}]Invalid key for
    \mpiconst{MPI_Info} 
    \begin{description}
    \item[\mpidconst{MPIi_ERR_INFO_KEY_NULL} \emsg{infokeynull}]Null key
    \item[\mpidconst{MPIi_ERR_INFO_KEY_LENGTH} \emsg{infokeylong}]Key
    is too long (optional 
      arguments: name, length, maxlength (string, int, int )
    \item[\mpidconst{MPIi_ERR_INFO_KEY_EMPTY}
    \emsg{infokeyempty}]Empty or blank key 
    \end{description}
\item[\mpiconst{MPI_ERR_INFO_VALUE} \emsg{infoval}]Invalid
    \mpiconst{MPI_Info} value 
    \begin{description}
    \item[\mpidconst{MPIi_ERR_INFO_VALUE_NULL} \emsg{infovalnull}]Null value
    \item[\mpidconst{MPIi_ERR_INFO_VALUE_LENGTH}
...
\item[\mpiconst{MPI_ERR_IO} \emsg{io}]Other I/O error (optional
  argument: text (string)) 
    \begin{description}
    \item[\mpidconst{MPIi_ERR_IO_ETYPE_FRACTIONAL} \emsg{ioetype}]Only
...
that is not consistent (string))
    \begin{description}
    \item[\mpidconst{MPIi_ERR_NOT_SAME_VALUE}
\emsg{notsamevalue}]Arguments to collective routine 
      ...
      (array of int))
    \item[\mpidconst{MPIi_ERR_NOT_SAME_ROOT}
      \emsg{notsameroot}]Inconsistent root 
    \item[\mpidconst{MPIi_ERR_NOT_SAME_COLLECTIVE_ORDER}
...
%    \item[\mpidconst{MPIi_ERR_}]
%    \end{description}
\item[\mpiconst{MPI_ERR_NO_SUCH_FILE} \emsg{fileexist}]File does not
...
\item[\mpiconst{MPI_ERR_PORT} \emsg{port}]Invalid port
    \begin{description}
    \item[\mpidconst{MPIi_ERR_PORT_EXIST} \emsg{portexist}]Named port
    does not exist (optional 
      arg: name of port (string))
    \item[\mpidconst{MPIi_ERR_PORT_TIMEOUT} \emsg{porttimeout}]Time
    out attempting a 
...
\item[\mpiconst{MPI_ERR_SERVICE} \emsg{servicename}]Invalid service name (see
  \mpifunc{MPI_Publish_name}) (optional arg: name (string))
    \begin{description}
    \item[\mpidconst{MPIi_ERR_SERVICE_UNPUBLISH}
...
\item[\mpiconst{MPI_ERR_SPAWN} \emsg{spawn}]Error in spawn call
    \begin{description}
    \item[\mpidconst{MPIi_ERR_SPAWN_FAILED} \emsg{spawnfail}]Could not
    spawn all requested processes 
    \item[\mpidconst{MPIi_ERR_SPAWN_NO_PGM} \emsg{spawnpgm}]The named
    program could not be 
      found (optional arg: name (spawn))
    \item[\mpidconst{MPIi_ERR_SPAWN_PROCESS_MNGER}
...
\item[\mpiconst{MPI_ERR_WIN} \emsg{win}]Invalid \mpiconst{MPI_Win}
    \begin{description}
    \item[\mpidconst{MPIi_ERR_WIN_NULL} \emsg{winnull}]Null
    \mpiconst{MPI_Win} 
    \item[\mpidconst{MPIi_ERR_WIN_NAME} \emsg{winname}]Cannot set
    window object name 
    \item[\mpidconst{MPIi_ERR_WIN_INVALID_WINDOW}
...
\item[\mpiconst{MPI_ERR_KEYVAL} \emsg{keyval}]Invalid keyval
    \begin{description}
    \item[\mpidconst{MPIi_ERR_KEYVAL_NULL} \emsg{keyvalnull}]Null
    keyval 
    \item[\mpidconst{MPIi_ERR_KEYVAL_FREE_PERM}
    \emsg{keyvalperm}]Cannot free permanent 
      attribute key
    \item[\mpidconst{MPIi_ERR_KEYVAL_NOT_IN_COMM}
    \emsg{keyvalcomm}]Keyval is not in communicator 
    \item[\mpidconst{MPIi_ERR_KEYVAL_NOT_IN_TYPE}
    \emsg{keyvaldtype}]Keyval is not in datatype 
    \item[\mpidconst{MPIi_ERR_KEYVAL_NOT_IN_WIN}
