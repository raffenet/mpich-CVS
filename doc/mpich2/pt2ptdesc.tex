...
\item The \code{busy} and \code{complete} flags in \code{MPID_Request} need be
...
\item The next test is for the special case of a contiguous buffer.  In this
  ...
  \mpidfunc{MPID_Memcpy} instead of \code{memcpy}.
...
\item Once the data has been moved from the eager buffer, the eager buffer 
 ...
 now marked as complete.
  ...
  a one in \code{complete} may mean ``data received but cleanup not done'' and
...
\item We need some way to indicate that the \code{MPID_Request} itself can be
  reused.  Setting the \code{complete} flag indicates that the user's buffer
...
\item In the multithreaded case but polling case, we use a single mutex on the
  agent.  Instead, we could have \code{GetNextPacket} set a mutex on that
...
\item In the multi-threaded case, if the request is not complete, we want to
  ...
  particular request.  (We are probably missing guards around \code{complete}
  elsewhere as well.)
...
\item Missing from this routine is any processing that may be required after
  ...
  \mpidfunc{MPID_Rhcv}, the \code{complete} field is set once data is
