...
appropriate operation?  E.g., a \mpidfunc{MPID_MemWrite_ordered} macro that takes
...
by the user, where size is (roughly) \code{count1 * datatype1->size}.  If the
...
complete (also using the \mpidfunc{MPID_MemWrite_ordered} macro).
...
\item All data has not been delivered.  In this case, a request to deliver the
  ...
  Question: should this simply be the index of the \mpiconst{MPI_Request}?
  ...
  like \code{MPI_Thread_initialize}).
...
\item The data can be sent eagerly.  This requires checking both the
  ...
  be sent) against the size of the message (\code{count2 * datatype2->size}).
...
Mark the send request as ready (\mpidfunc{MPID_MemWrite_ordered}) and use
...
\end{verbatim}  
The \code{pthread_cond_wait} atomically releases the \code{request_mutex} and
...
\item A matching request was not found.  Then, as in the \mpifunc{MPI_Irecv}
  case, \mpidfunc{MPID_Request_recv_FOA} will have created a new request and
...
\item Consult \mpidconst{MPID_Segment} for the next group of bytes
  ...
  location with \mpidfunc{MPID_Segment_pack} (if the user's buffer \emph{is}
...
copying in the contiguous, and with more work, the \code{iovec}, case).
...
to wait for incoming information.  Note that Rob prefers \code{poll}.
Question: Can we use \code{poll} always for Unix?  Should we?  If not, how do
we want to choose between \code{poll} and \code{select}?
...
\end{enumerate}
A further refinement is to allow the use of \code{readv} once the read buffer
is flushed.  

Question: We could use \code{MSG_PEEK} and \code{recv} instead of \code{read}
...
\implementation{\mpifunc{MPI_Irecv}}
\begin{verbatim}
err = MPI_SUCCESS;
/* Error checking */
...
...
\implementation{\mpifunc{MPI_Isend}}
\begin{verbatim}
/* Error checking */
...
...
\implementation{\texttt{Communication Agent}}
