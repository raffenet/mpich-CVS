...
\subsubsection{Q: What is MPICH2?}
...
\subsubsection{Q: What does MPICH stand for?}
...
\subsection{Building MPICH2}
\subsubsection{Q: What is the difference between the MPD and SMPD process managers?}
...
\subsubsection{Q: Do I have to configure/make/install MPICH2 each time for each  compiler I use?}
...
In some cases, MPICH2 is able to build the Fortran interfaces in a way that
 ...
 the default.
\subsubsection{Q: How do I configure to use the Absoft Fortran compilers?}
...
setenf F90 f90
\end{verbatim}
\subsubsection{Q: When I configure MPICH2, I get a message about FDZERO and the configure aborts}
...
\subsubsection{Q: When I use the g95 Fortran compiler on a 64-bit platform, some  of the tests fail}
...
\subsection{Windows version of MPICH2}
\subsubsection{I am having trouble installing and using the Windows version of MPICH2}
See the
...
\subsection{Compiling MPI Programs}
\subsubsection{C++ and \texttt{SEEK\_SET}}
Some users may get error messages such as
\begin{small}
\begin{verbatim}
    SEEK_SET is #defined but must not be for the C++ binding of MPI
...
to the command line (this will cause the MPI versions of \texttt{SEEK\_SET}
etc. to be skipped).
\subsubsection{When building the ssm or sshm channel, I get the error ``mpidu\_process\_locks.h:234:2: error: \#error *** No atomic memory operation specified to implement busy locks ***''}
...
\subsection{Running MPI Programs}
\subsubsection{Q: How do I pass environment variables to the processes of my parallel program}
A: The specific method depends on the process manager and version of 
\texttt{mpiexec} that you are using. 
\subsubsection{Q:  How do I pass environment variables to the processes of my parallel program when using the mpd process manager?}
...
\subsubsection{Q:  What determines the hosts on which my MPI processes run?}
...
\subsubsection{Q: How do I run MPI programs in the background when using the default MPD process manager?}
