...
overhead).  These are the \code{MPID_PE_poll_xxx} routines (see
Section~???).
...
\begin{enumerate}
\item \code{mpiexec}.  How does MPICH create the correct \code{mpiexec}?
\item Installation.  How does MPICH get the proper device-specific files
  installed; e.g., the \code{mpd} for the \code{mpd} device?  There
should probably be \code{install} targets in the device's top-level
\file{Makefile}.  
\item Device-specific documentation, such as environment variables and
  command-line arguments used only by a particular device.  This
should probably be placed into a specific file, such as \file{devdoc.txt}.
\item Testing codes for device-specific functions.  These should be
placed in a \file{test} subdirectory of the device, and the tests
...
superceeded?  Are streams used to implemente xfer?}
...
error codes and classes, we will probably allow 255 classes and 255
