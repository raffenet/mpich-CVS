...
\begin{enumerate}
\item \code{MPI_Irsend( MPI_BOTTOM, 1, my_indexed_datatype, ... )}
where the datatype describes 50MB of data, each entry of which is a
structure.
\item \code{MPI_Accumulate( MPI_BOTTOM, 1, my_indexed_datatype, ..., 
   my_vector_type, ..., MPI_MAXLOC )}, where \code{my_vector_type} is
