...
\begin{verbatim}
/* Routines that create and free requests */
...
void CH3_free_request( MPID_Request * );
...
(not written yet)
This section should describe the queue operation routines.

\section{Pseudo-code for some of the \code{MPID\_} routines}
This section outlines the pseudocode for some of the major MPID routines.
...
is oterwise not available.  Also note that this routine cannot be
...
when \code{select} or \code{poll} returns.  Should we this in the code?
...
to guarantee exclusive access to the data structures.
...
<MPICH keyword=value />
\end{verbatim}
and have both the first and second choices.
...
used instead of a \code{buf,count} pair.  The special acction on a
