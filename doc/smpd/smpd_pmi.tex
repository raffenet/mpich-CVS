...
key=rank
value=nproc
node_id=smpd node id

The init command is sent to the root from each process.  This lets mpiexec know
that the processes have started and made it to PMI_Init
\item[\texttt{finalize}]\mbox{}\\
name=kvsname
key=rank
node_id=smpd node id

The finalize command is sent to the root from each process when PMI_Finalize is called.
...
\item[\texttt{abort_job}]\mbox{}\\
name=kvsname
rank=rank
error=error message
exit_code=exit code

abort_job is sent from any node to the root to abort the entire job.  This
command represents a user abort usually caused by an MPI_Abort call.
\end{description}
