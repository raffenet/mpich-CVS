...
The product GUIDs need to be changed when a new release is created.  To do this open DevStudio
and go to the installation project.  Change the version from 1.0.x to 1.0.y.  DevStudio will
prompt you to change the GUIDs and click yes.

Then you can build a distribution.  The GUIDs only need to be changed when a new version is 
released to the public.  Release candidates can have the same GUIDs.  So maybe the best policy
would be to change the GUIDs immediately after a release so they would stay the same all the
way up to and including the next release.
...
will be done without any passwords.
