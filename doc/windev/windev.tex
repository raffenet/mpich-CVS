...
MPICH2 suite comes with the Multi-Processing Environment (MPE) which attempts to provide programmers
with a complete suite of performance analysis tools.
...
A developer can trace the MPI calls using the tracing functionality of the mpiexec
...
\begin{enumerate}
	...
	\item After launching the mpiexec wrapper, select the application that you would
			like to run and select the number of processes. Now click on "more options"
			checkbox to show the extended options for mpiexec command.
	...
	\item Open your logfile (the default name of the logfile is <APPLICATION NAME>.clog2) 
...
\begin{enumerate}
	...
	\item Launch jumpshot using the java command. 
			(eg: java -jar "c:\Program Files\MPICH2\bin\jumpshot.jar")
...
\begin{enumerate}
\item Get handles from MPE logging library: MPE_Log_get_state_eventIDs()
   ...
   PS. Hardwiring the eventIDs is considered a big idea since it may cause
   eventID confict and so the practice isn't supported.  Older MPE library
   provides MPE_Log_get_event_number() which is still being supported but
   has been deprecated, users are strongly urged to use
   MPE_Log_get_state_eventIDs() instead.

\item Set the logged state's characteristics: MPE_Describe_state() sets the
   name and color of the states.

\item Log the events of the logged states: MPE_Log_event() are called twice
   to log the user-defined states.
Below is a simple example of that uses the 3 steps outlined above.
...
event which can be defined through use of MPE_Log_get_solo_eventID()
