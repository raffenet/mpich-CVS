...
supported are sock, ssm, shm, and sshm.  These represent the sockets, sockets 
plus shared memory, shared memory, and scalable shared memory channels.  The
shared memory channels only work on one node.  The sockets and sockets plus
shared memory channels work on multiple nodes.  There are also profiled versions
of the channels that produce RLOG files for each process when selected.  They
are named p, ssmp, shmp, and sshmp.  See the section on channel selection for
